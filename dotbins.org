...
#+begin_src ruby :tangle (expand-tangle-target ".local/bin/mediautil")
  ...
  class MediaUtil < Thor
    ...
    def dvdencode()
      ...
      if FileType.block_device?(equip.in_file) ||
        ...
        if options[:aencoder] || options[:aquality] || options[:abitrate]
  ...
  class AudioConfig
    attr_accessor :encoder, :quality, :bitrate

    DEFAULT_ENCODER = 'libopus'
    DEFAULT_QUALITY = '100'
    DEFAULT_BITRATE = '128'

    def initialize(encoder, quality=nil, bitrate=nil)
      @encoder = (encoder ? encoder : DEFAULT_ENCODER)
      @quality = (quality ? quality : DEFAULT_QUALITY)
      @bitrate = (bitrate ? bitrate : DEFAULT_QUALITY)
    end
      ...
      outfilep = proc do |fname, optset|
        ...
        optset.each do |opt|
          if opt.instance_of?(VideoConfig)
            oopts << format("-vcodec %s", opt.encoder)
            ...
            oopts << format("-acodec %s", opt.encoder) if opt.encoder
            oopts << format("-aq %s", opt.quality) if opt.quality
            oopts << format("-ab %s", opt.bitrate) if opt.bitrate
