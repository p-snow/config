...
#+begin_src ruby :tangle (expand-tangle-target ".local/bin/mediautil")
  ...
  class MediaUtil < Thor
    ...
    def encode()
      ...
      if options[:vencoder] || options[:vquality]
        vconf = VideoConfig.new(options[:vencoder], options[:vquality])
      end
      if options[:aencoder] || options[:aquality]
        aconf = AudioConfig.new(options[:aencoder], options[:aquality])
      end
    ...
    option :format, aliases: 'f', :type => :string,
           :banner => 'ogg,m4a', :default => 'm4a',
           :desc => 'comma separated encoding formats'
    def cdencode()
      commander = Commander.new(options[:log], options[:dryrun])
    ...
    def dvdencode()
      ...
      if FileType.block_device?(equip.in_file) ||
        ...
        configs = []
        if options[:vencoder] || options[:vquality]
          configs << VideoConfig.new(options[:vencoder], options[:vquality])
        end
        if options[:aencoder] || options[:aquality]
          configs << AudioConfig.new(options[:aencoder], options[:aquality])
        end
        dconf = DVDConfig.new
  ...
  class VideoConfig
    attr_accessor :encoder, :quality, :preset, :tune, :profile, :level

    DEFAULT_ENCODER = 'x265'
    DEFAULT_QUALITY = '26.0'

    def initialize(encoder, quality)
  ...
  class AudioConfig
    attr_accessor :encoder, :quality

    # DEFAULT_ENCODER = 'fdk_aac'
      ...
      outfilep = proc do |fname, optset|
        ...
        optset.each do |opt|
          if opt.instance_of?(VideoConfig)
            oopts << format("-vcodec lib%s", opt.encoder)
            ...
            oopts << format("-acodec lib%s", opt.encoder) if opt.encoder
            oopts << format("-aq %s", opt.quality) if opt.quality
    ...
    def self.command(input, output, configs, raw_option)
      ...
      configs.each do |config|
        if config.instance_of?(VideoConfig)
          cmd_opts << format("--encoder %s", config.encoder)
          ...
          cmd_opts << format("--aencoder %s", config.encoder)
