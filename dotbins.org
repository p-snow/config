...
- back_btrfs.sh
  #+begin_src shell :tangle (expand-tangle-target ".local/bin/back_btrfs.sh")
    ...
    src_dev=/mnt/ssd01
    src_snap_dir="${src_dev}"/snapshots/doc
    src_back_dir="${src_dev}"/backups/doc
    dst_dev=/mnt/hdd01
    dst_back_dir="${dst_dev}"/backups/doc

    parent=$(ls -d "${src_back_dir}"/* 2>/dev/null | sort | tail -1 | xargs basename)
    snap_earliest=$(ls -d "${src_snap_dir}"/* 2>/dev/null | sort | head -1 | xargs basename)
    snap_latest=$(ls -d "${src_snap_dir}"/* 2>/dev/null | sort | tail -1 | xargs basename)
    ...
    if [ -n "${parent}" ] && [[ "${parent}" < "${snap_latest}" ]]; then
      ...
      if type sudo btrfs subv show "${dst_back_dir}/${parent}" >/dev/null 2>&1 ; then
        pwsudo btrfs send -p "${src_back_dir}/${parent}" "${src_back_dir}/${snap_latest}" | sudo btrfs receive "${dst_back_dir}"
      else
        pwsudo btrfs send "${src_back_dir}/${snap_latest}" | sudo btrfs receive "${dst_back_dir}"
      fi
    fi
  #+end_src
- back_rsync.sh
