...
-   Changes are applied by editing the `.org`{.verbatim} files, which
    are then tangled into the actual configuration files (e.g.,
    `.zshrc`{.verbatim}, `init.lua`{.verbatim}).
...
-   `style`{.verbatim}: Formatting or cleanup of the Org-mode file (does
...
-   `docs`{.verbatim}: Adding explanations or documentation within the
    Org-mode file.
...
1.  **Format**: Follow the \[Conventional Commits
    ...
    specification.
2.  **Scope**: Use the application or category name as the scope (e.g.,
    `feat(zsh): ...`{.verbatim}, `fix(nvim): ...`{.verbatim}).
