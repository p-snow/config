  ...
  (setq org-web-track-trackers
        `((,(rx "www.amazon.co.jp"
          ...
          (,(regexp-quote "www.amazon.co.jp")
           "pup 'span[id=\"priceblock_ourprice\"],span[id=\"kindle-price\"],span[class=\"a-text-price\"],span[class=\"a-offscreen\"] json{}' \| jq -re '.[0].text'" ; price
           "pup 'div[id=\"availability\"] json{}' \| jq -re '.[0].children[0].text'") ; availability
          (,(regexp-quote "www.yodobashi.com/product")
           "pup 'span[id=\"js_scl_unitPrice\"] json{}' \| jq -r .[0].text" ; price
           "pup 'span[id=\"js_scl_pointValue\"] json{}' \| jq -r .[0].text") ; point
  ...
  (defun org-web-track-get-values (url &optional sync on-success on-fail &rest args)
    ...
    (let ((tracker-def (assoc-default url org-web-track-trackers #'string-match))
          ...
          (request-curl-options
           `(,(format "-H \"User-Agent: %s\"" (string-trim (url-http-user-agent-string)))))
          (values nil))
      (unless tracker-def
