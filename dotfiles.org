...
#+begin_src emacs-lisp :tangle no :tangle (expand-file-name "tempel/templates/user/guix.eld" tangle/emacs-etc-dir)
...
(home-environment
 ...
 (packages (list glibc-locales fontconfig which pinentry
...
#+begin_src shell :tangle (expand-file-name "activate-home" tangle/local-bin-dir)
guix home container ~/.config/guix/home/home-config.scm
#+end_src
...
- gpg-agent config
  ...
  pinentry-program <<which(filename="pinentry")>>
...
**** variables for special needs

***** user elisp code directory

This variable points to the directory where user's Elisp code is stored. The reason why the path starts with "p-snow/dotfiles" instead of simply "~/.config/emacs" is to accommodate the org-babel-tangle-jump-to-org command.

#+begin_src emacs-lisp :noweb tangle
(defvar dotfiles/user-elisp-dir
  (file-name-concat
   "<<ghq-proj-path(projname="p-snow/dotfiles")>>"
   ".config/emacs/lisp/tangled"))
#+end_src

***** user tempel template directory

This variable points to the location where user TempEL templates are stored.

#+begin_src emacs-lisp :noweb tangle
(defvar dotfiles/user-template-dir
  (file-name-concat
   "<<ghq-proj-path(projname="p-snow/dotfiles")>>"
   ".config/emacs/etc/tempel/templates/user"))
#+end_src
...
#+begin_src emacs-lisp
(transient-define-prefix ghq-get-dispatch-dwim (&optional url)
  "Dispatch ghq get command."
  :man-page "du"
  [("-s" "shallow clone" "--shallow"
    :init-value (lambda (obj) (oset obj value "--shallow")))]
  [(dispatch-dwim-execute
    :command-format
    (format "ghq get %%a %s"
  ...
  (transient-setup 'ghq-get-dispatch-dwim nil nil
                   :scope (plist-put nil 'url (dispatch-dwim--determine-url url))))
#+end_src
...
(cd /mnt/exdoc/doc/ && git rev-parse --is-inside-work-tree 2>/dev/null && git annex sync && git annex get)
(cd /mnt/vt-m/doc/ && git rev-parse --is-inside-work-tree 2>/dev/null && git annex sync && git annex get)
(cd /mnt/vt-h/doc/ && git rev-parse --is-inside-work-tree 2>/dev/null && git annex sync && git annex get)
(cd /mnt/vt-j/doc/ && git rev-parse --is-inside-work-tree 2>/dev/null && git annex sync && git annex get)
(cd <<doc-dir()>> && git rev-parse --is-inside-work-tree 2>/dev/null && git annex sync)

(cd /mnt/exdoc/doc/ && git rev-parse --is-inside-work-tree 2>/dev/null && git annex sync)
#+end_src
...
#+NAME: ghq-proj-path
#+HEADER: :var projname="p-snow/guix-channel"
