...
**** shell command builder using transient
:PROPERTIES:
...
(transient-define-suffix my/command-builder-execute ()
  :transient 'transient--do-exit
  :key "x"
  :description "execute"
  (interactive)
  (detached-shell-command (my/command-builder--build) t))

(transient-define-suffix my/command-builder-insert ()
  :transient 'transient--do-exit
  :key "i"
  :description "insert"
  :if-derived 'minibuffer-mode
  (interactive)
  (insert (my/command-builder--build)))

(defun my/command-builder--build ()
  ""
  (flet ((exp-fun (arg)
                  (pcase arg
                    ((and (pred stringp) file) file)
                    ((and (pred listp) li)
                     (mapconcat 'exp-fun li " ")))))
    (let* ((comm-data (oref transient-current-prefix scope))
           (prefix-val (oref transient-current-prefix value))
           (fmt-str (or (and prefix-val
                             (exp-fun prefix-val))
                        (plist-get comm-data 'format))))
      (format
       (if (string-match-p "%i" fmt-str)
           (replace-regexp-in-string
            "%i"
            (if (eq minibuffer-history-variable 'dired-shell-command-history)
                "*"
                                ...
                                (pcase arg
                                  ((and (pred stringp) file)
                                   (if (file-exists-p file)
                                     ...
                                     file))
                                  ((and (pred listp) li)
                                   (mapconcat 'quote-str li " ")))))
                (quote-str (or (plist-get comm-data 'input)
...
#+begin_src emacs-lisp
(cl-defun command/mpv (&key file no-resume start end)
  ""
  (interactive)
  (transient-define-prefix my-shell-command/mpv ()
    "Prefix that just shows off many typical infix types."
    :value "mpv %s %i"
    [("-R" "noresume" "--no-resume-playback"
      :init-value (lambda (obj) (oset obj value
                                  (if no-resume "--no-resume-playback" nil))))
    ...
    [(my/command-builder-execute)
     (my/command-builder-insert)])
  (transient-setup 'my-shell-command/mpv nil nil
                   :scope (when file
                            (plist-put nil 'input file))))
...
(with-eval-after-load 'embark
  (mapc (lambda (map)
          (lambda-key map "P"
                      (lambda (file)
                        "play media FILE"
                        (command/mpv :file file :no-resume t))))
        (list embark-file-map embark-url-map))
  (with-eval-after-load 'org
    (mapc (lambda (map)
            (lambda-key map "P"
                     ...
                     (command/mpv :file
                                  (mapcar (lambda (item)
                                            (when (string-match org-link-any-re (car item))
                                              (url-filename (url-generic-parse-url
                                                             (org-link-expand-abbrev
                                                              (match-string-no-properties 2 (car item)))))))
                                          (cdr (org-list-to-lisp list)))
                                  :no-resume t))))
     (list embark-org-plain-list-map))))
#+end_src

***** command: produce opus audio
:PROPERTIES:
:CREATED:  [2023-11-12 Sun 22:00]
:END:

#+begin_src emacs-lisp
(cl-defun command/produce-opus (&key file bitrate tempo)
  ""
  (interactive)
  (transient-define-prefix my-shell-command/mpv ()
    "Prefix that just shows off many typical infix types."
    [("-b" "bitrate" "-b:a " :prompt "bitrate: " :class transient-option
      :init-value (lambda (obj) (oset obj value (or bitrate "64k"))))
     ("-t" "tempo" "-filter:a atempo=" :prompt "tempo: "
      :init-value (lambda (obj) (oset obj value tempo)))]
    [(my/command-builder-execute)
     (my/command-builder-insert)])

  (transient-setup 'my-shell-command/mpv nil nil
                   :scope (let ((command-data (plist-put nil 'format
                                                         "parallel ffmpeg -i '{}' -c:a libopus -ac 2 %s '{.}'.opus ::: %i")))
                            (if file
                                (plist-put command-data 'input file)
                              command-data))))
#+end_src

***** command: system sleep
:PROPERTIES:
:CREATED:  [2024-01-20 Sat 14:29]
:END:

#+begin_src emacs-lisp
(transient-define-prefix my/system-sleep ()
  "Prefix that just shows off many typical infix types."
  [("-d" "date" "--date " :prompt "When to wake up?: " :class transient-option
...
(defun my/transient-read-time (prompt default-time _history)
  ...
  (format "'%s'"
          (or (seq-some (lambda (d)
                          (let ((time-str (string-trim
                            ...
                            (unless (string-prefix-p "date: invalid date"
                                                     time-str)
                              time-str)))
                        (delq nil (list (and prompt (read-string prompt)) default-time)))
              "")))

(defun command/system-sleep (sec)
  ""
  (interactive "sSleep for (sec[s/m/h/d]): ")
  (transient-setup 'my/system-sleep nil nil
                   :scope (plist-put nil 'format
                                     (format "sleep %s; pwsudo rtcwake -l -m mem -v %%s; %s"
                                             sec
                                             (completing-read "Wake-up task: "
                                                              '("morning.sh;"
                                                                "which system.sh | xargs pwsudo; morning.sh; weekend.sh;"))))))
#+end_src
...
#+name: command/pandoc
#+begin_src emacs-lisp
(transient-define-prefix my/pandoc-menu ()
  ""
  :value '("parallel pandoc %s '{}' ::: %i")
  [(my/pandoc-to)]
  [(my/command-builder-execute)
   (my/command-builder-insert)])
...
(cl-defun my/pandoc (&key file)
  ""
  (interactive)
  (transient-setup 'my/pandoc-menu nil nil
  ...
  (let ((ask-file (when (or (file-directory-p media)
    ...
    (when (or (not ask-file)
      ...
      (apply 'funcall-interactively
             'command/mpv :file media
             (and (stringp query)
                  (flatten-list
                   (mapcar (lambda (cons-cell)
                             (pcase cons-cell
                               (`(,void-var "") `(:start ,void-var))
                               (`(,(or "s" "start") . ,val)
                                `(:start ,@val))
                               (`(,(or "e" "end") . ,val)
                                `(:end ,@val))))
