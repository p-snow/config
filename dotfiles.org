...
#+PROPERTY: header-args:emacs-lisp :tangle-mode o444
...

vz7VQ28qvvrH9vKNlqXv
=hkcg
-----END PGP MESSAGE-----
...

4TgmWcngvArMI1W7N34SinWf
=AHjv
-----END PGP MESSAGE-----
...
- Automating Script
  ...
  #+begin_src shell :eval no :tangle no :noweb-ref weekend-batch
    update-channels && activate-profiles desktop emacs && guix gc -d 2m -F 20G
  #+end_src
...
#+begin_src bash :eval no :comments no :noweb tangle :tangle (expand-file-name ".local/bin/cron-reinit.sh")
  crontab -r
...
#+begin_src conf :noweb tangle :tangle (expand-file-name ".config/btrbk/btrbk.conf")
  ...
  volume /mnt/home
    target /mnt/ex02/pc01
    subvolume home
#+end_src

**** snapshot/backup script

#+begin_src shell :noweb tangle :tangle (expand-file-name ".local/bin/btrbk_home") :shebang #!/usr/bin/env bash :tangle-mode (identity #o755)
  BTRBK_CONF=~/.config/btrbk/btrbk.conf

  pwsudo btrbk --config "${BTRBK_CONF}" run
#+end_src

#+begin_src conf :noweb yes :noweb-ref cronjob-user
  ,*/15 * * * * /bin/bash -ci "btrbk_home >> $LOG_DIR/btrbk_home.log 2>&1"
#+end_src
...
#+begin_src elisp :eval no-export
  (let ((auto-save-dir (file-name-concat user-emacs-directory
                                         "auto-save")))
...
#+begin_src emacs-lisp :noweb tangle
  ...
  (setopt auto-save-visited-interval 20)
...
***** extract_headings
#+begin_src python :tangle (expand-file-name ".local/bin/extract_headings") :tangle-mode (identity #o755) :shebang #!/usr/bin/env python3
            ...
            (cond
             ...
             ((member todo-state
              ...
              (format "%s::* suspended tasks"
                      (denote-get-path-by-id "20230605T180351"))))))
...
#+begin_src emacs-lisp
  (use-package ob-core
    ...
    (setq org-babel-ruby-command (executable-find "ruby"))
    (cond
...
#+begin_src emacs-lisp :tangle (expand-file-name "my-org-web-track.el" my/user-config-dir)
...
#+begin_src emacs-lisp :noweb tangle :tangle (expand-file-name "batch/org-web-track-mail.el" my/user-config-dir)
  (provide 'my/org-mode-init)
...
#+name: openai-api-key
#+begin_src emacs-lisp
  (password-store-get-field "openai.com" "API key")
#+end_src
...
#+begin_src emacs-lisp
  (defun my/delete-char-dwim ()
    ...
    (if (eq major-mode 'org-mode)
        (org-delete-char 1)
      (delete-char 1)))
  (defun my/delete-cackward-char-dwim ()
    (interactive)
    (if (eq major-mode 'org-mode)
...
elfeed-update will run at every midnight time and if Emacs idle time elapse 1.8 hours.
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-x C-#") #'my/sudo-edit)
#+end_src
...
:header-args+: :tangle (expand-file-name "my-org-config.el" my/user-config-dir)
:END:
***** search org headings under the directory with consult
:PROPERTIES:
:CREATED:  [2023-10-20 Fri 20:14]
:END:

#+begin_src emacs-lisp
  (defun my/consult-org-under (dir)
    "Search org heading from all org files under DIR."
    (interactive "sDir: ")
    (when-let* ((dir-fixed (replace-regexp-in-string "\\(?:^/~\\)" "~"
                                                     dir))
                (dir-expanded (expand-file-name dir-fixed))
                (org-agenda-files
                 (and (file-directory-p dir-expanded)
                      (directory-files-recursively dir-expanded
                                                   "\\(?:\\.org\\(?:\\.gpg\\)?$\\)"))))
      (consult-org-agenda)))

  (with-eval-after-load 'embark
    (keymap-set embark-file-map "H"
                #'my/consult-org-under))
...
#+begin_src shell :tangle (expand-file-name ".local/bin/volget") :tangle-mode (identity #o755) :shebang #!/usr/bin/env bash
...
#+begin_src emacs-lisp
  (use-package org-ql-view
    ...
    (push `("Someday tasks"
            ...
            ,(append
              (org-agenda-files)
              (list (denote-get-path-by-id "20230605T180351")))
...
#+begin_src emacs-lisp
  ...
  (defvar my/org-remove-old-blobs-from 21
  ...
  (defun my/org-remove-old-clock-logs ()
    "Remove all clocking entries but last `my/org-remove-old-blobs-from'."
    (interactive)
    (org-save-outline-visibility t
      (save-excursion
        (org-back-to-heading t)
        (org-show-all)
        (when-let* ((drawer (re-search-forward org-clock-drawer-start-re (save-excursion (org-end-of-subtree)) t))
                    (case-fold-search t)
                    (re org-clock-line-re)
                    (end-mkr (set-marker (mark-marker) (save-excursion (re-search-forward org-clock-drawer-end-re (save-excursion (org-end-of-subtree)) nil))))
                    (count 0))
          (while (re-search-forward re (marker-position end-mkr) t)
            (skip-chars-forward " \t\r")
            (looking-at org-tr-regexp-both)
            (if (>= count my/org-remove-old-blobs-from)
                (kill-whole-line)
              (setq count (1+ count)))))
  ...
  (defun my/org-remove-old-state-changes ()
    "Remove all state change entries but last my/org-remove-old-blobs-from."
    (interactive)
    (org-save-outline-visibility t
      (save-excursion
        (org-back-to-heading t)
        (org-show-all)
        (when-let* ((drawer-end (save-excursion (re-search-forward org-logbook-drawer-re
...
#+begin_src emacs-lisp  :tangle (expand-file-name "org-drill-english.el" my/user-package-dir) :comments no
  ;; -*- lexical-binding: t -*-
...
#+begin_src shell :noweb tangle :tangle (expand-file-name ".local/bin/updatedb-home") :tangle-mode (identity #o555) :shebang #!/usr/bin/env bash
  . ~/.bashrc
...
- festival.el
  ...
  #+begin_src emacs-lisp :noweb-ref elisp-snip
...
#+begin_src bash :noweb tangle :tangle (expand-file-name ".local/bin/midnight.sh")
  <<midnight-batch>>
#+end_src

#+begin_src conf :noweb yes :noweb-ref cronjob-user
  0 4 * * * /bin/bash -ci midnight.sh
#+end_src
...
#+begin_src bash :noweb tangle :tangle (expand-file-name ".local/bin/weekend.sh")
  <<weekend-batch>>
#+end_src

#+begin_src conf :noweb yes :noweb-ref cronjob-user
  30 2 * * 6 /bin/bash -ci weekend.sh
#+end_src
...
#+begin_src shell :noweb tangle :tangle (expand-file-name ".local/bin/vp9transc") :tangle-mode (identity #o755) :comments no
  <<transc-shared>>
...
#+begin_src shell :noweb tangle :tangle (expand-file-name ".local/bin/av1transc") :tangle-mode (identity #o555) :comments no
  <<transc-shared>>
...
#+begin_src shell :tangle (expand-file-name ".local/bin/opusencode") :tangle-mode (identity #o555) :shebang #!/usr/bin/env bash
  set -euo pipefail
...
#+begin_src shell :tangle (expand-file-name ".local/bin/dvdpack") :tangle-mode (identity #o555) :shebang #!/usr/bin/env bash
  set -euo pipefail
...
#+begin_src conf :noweb-ref cronjob-user
...
#+begin_src shell :noweb tangle :tangle (expand-file-name ".local/bin/git-sync") :tangle-mode (identity #o755) :shebang #!/usr/bin/env bash
  #
  # git-sync
  #
...
#+begin_src ruby :tangle (expand-file-name ".local/bin/mediautil") :shebang #!/usr/bin/env ruby
  require 'thor'
  require 'open3'
  require 'fileutils'
  require 'tempfile'
...
#+begin_src ruby :tangle (expand-file-name ".local/bin/genalbum") :shebang #!/usr/bin/env ruby
  require 'fileutils'
  require 'time'
  require 'exifr'
...
#+begin_src python :noweb tangle :tangle (expand-file-name ".local/bin/sendmail.py") :shebang #!/usr/bin/env python3
  import sys
  import argparse
  import subprocess
  import smtplib
...
#+begin_src python :tangle (expand-file-name ".local/bin/strip_ruby") :tangle-mode (identity #o755) :shebang #!/usr/bin/env python3
  import sys
  import lxml.html as html
