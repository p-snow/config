...
#+begin_src conf :noweb tangle :tangle (expand-file-name "shared/etc/keyd/default.conf" tangle/system)
[ids]
  ...
  (let ((entries (elfeed-search-selected)))
    (cl-loop for entry in entries
             ...
             do (let ((title (elfeed-entry-title entry)))
                  ...
                  (my/shell-command-execute-macro)))
...
(use-package org-web-track
  :ensure t
  ;; :load-path "~/data/home/.local/share/ghq/github.com/p-snow/org-web-track"
...
[[https://github.com/karthink/gptel][gptel]] is my go-to AI frontend in Emacs as of [2024-07-19 Fri]. It supports a large number of services (LLM models).

#+begin_src emacs-lisp :noweb tangle
(use-package gptel
  :ensure t
  :after engine-mode
  :bind ("H-Q" . my/gpeel-invoke)
  :preface
  (defun my/gpeel-invoke (&optional arg)
    "Enable users to call gptel/gptel-menu/gptel-send by changing prefix argument."
    (interactive "P")
  ...
  (cl-pushnew '(proofreading . "Please help me proofread the following text with English.")
              gptel-directives)
  (cl-pushnew '(commit-message-pr . "Please help me proofread the following text as a commit message for Git.")
              gptel-directives)
...
**** Shell commands with transient menu
:PROPERTIES:
...
(defvar my/shell-command-force-action nil)
(defvar my/shell-command-buffer "*Shell Command*")

(defclass my/shell-command (transient-suffix)
  ((command-format :initarg :command-format :initform "%a")
   (command-format-spec :initarg :command-format-spec :initform nil)))

(cl-defmethod transient-init-value ((obj my/shell-command))
  (unless (slot-boundp obj :command-format)
    (oset obj command-format "%a")))

(defvar my/transient-common-menu
  ["Action"
   ("M-w" "Copy the command to kill-ring" "M-w")
   ("M-i" "Insert the command to dedicated buffer" "M-i")
   ("M-x" "Execute the command" ""
    :class transient-switches
    :argument-format "%s"
    :argument-regexp "\\(async\\|sync\\)"
    :choices ("async" "sync")
    :init-value (lambda (obj) (oset obj value "async")))])

(transient-define-suffix my/shell-command-action (args)
  :class my/shell-command
  ;; :transient 'transient--do-return
  :key "RET" :description "Take action"
  (interactive (list (transient-args transient-current-command)))
  (let* ((obj (transient-suffix-object))
         (command (my/transient-shell-command obj))
         (action (or my/shell-command-force-action
                     (seq-some (lambda (val)
                                 (and (member val '("M-w" "M-i" "async" "sync"))
                                      val))
                               args))))
    (pcase action
      ("M-w"
      ...
      ("M-i"
       (and (length> command 0)
            (with-current-buffer (get-buffer-create my/shell-command-buffer)
              (insert command "; ")
...
(defun my/transient-shell-command (obj)
  (let* ((command-format-spec (and (slot-boundp obj :command-format-spec)
...
(defun my/transient-suffix (key)
  ""
  (seq-find (lambda (obj) (equal (oref obj key) key))
            (or transient--suffixes transient-current-suffixes)))

(defun my/transient-infix-values (keys)
  (mapconcat 'transient-infix-value
             (mapcar #'my/transient-suffix keys)
             " "))

(defmacro my/determine-file (&optional multi)
  ""
  `(pcase (cond
...
(defmacro my/determine-url ()
  ...
  '(let ((url-alike (cond
                     ...
                     ((with-temp-buffer
                        (insert-for-yank (car kill-ring))
                        (buffer-string))))))
     (if (string-match-p ffap-url-regexp url-alike)
         url-alike
       (read-string "URL: "))))

;; Readers

(defun my/transient-read-shell-command (&optional prompt default-time history)
  ""
...
(defun my/transient-output-reader (&optional prompt initial-input history)
  ""
  (let* ((infile-key-obj (my/transient-suffix (plist-get (oref transient--prefix scope) :infile-key)))
...
(defalias 'my/shell-command-execute-macro (kmacro "RET"))
#+end_src
...
#+begin_src emacs-lisp
(transient-define-prefix my/mpv-command (file &optional no-resume start end)
  ""
  :man-page "mpv"
  ["Options"
  ...
  ["Inputs"
   ("f" "files" ""
    :class my/shell-command-file-option
    :multi-value t
    :init-value (lambda (obj) (oset obj value (ensure-list file))))]
  [my/transient-common-menu
   [(my/shell-command-action
     :command-format "mpv %a")]]
  (interactive `(,(my/determine-file t)))
  (transient-setup 'my/mpv-command))

(defclass my/shell-command-file-option (transient-option) ())

(cl-defmethod transient-infix-value ((obj my/shell-command-file-option))
  ""
  (and-let* ((value (oref obj value)))
    (let ((arg (oref obj argument)))
      (pcase-exhaustive (oref obj multi-value)
        ('nil          (concat arg (shell-quote-argument value)))
        ((or 't 'rest) (cons arg (mapcar 'shell-quote-argument value)))
        ('repeat       (mapcar (lambda (v) (concat arg (shell-quote-argument v))) value))))))
...
(transient-define-prefix my/ffmpeg-libopus-command (file)
  ...
  ["Outfile"
   ("-o" "output" ""
    :class transient-option
    :reader my/transient-output-reader
    :always-read t
    :init-value (lambda (obj) (oset obj value (my/transient-output-reader nil "${f%.*}.opus"))))]
  [my/transient-common-menu
   [(my/shell-command-action
     :command-format
     "ffmpeg %i %O %o"
     :command-format-spec ((?i . (my/transient-infix-values '("-i")))
                           (?O . (my/transient-infix-values '("a" "-B" "-t")))
                           (?o . (my/transient-infix-values '("-o")))))]]
  (interactive (list (my/determine-file nil)))
...
(transient-define-prefix my/ffmpeg-libvp9-command (file)
  ...
  ["Outfile"
   ("-o" "output" ""
    ...
    :reader my/transient-output-reader
    :always-read t
    :init-value (lambda (obj) (oset obj value (my/transient-output-reader nil "${f%.*}_vp9.mkv"))))]
  [my/transient-common-menu
   [(my/shell-command-action
     :command-format
     (concat "ffmpeg %i %O %o /dev/null && ffmpeg %i"
             ...
             "-ac 2 %O %o")
     :command-format-spec ((?i . (my/transient-infix-values '("-i")))
                           (?O . (my/transient-infix-values '("-c" "a" "-B")))
                           (?o . (my/transient-infix-values '("-o")))))]]
  (interactive (list (my/determine-file nil)))
...
(transient-define-prefix my/ffmpeg-libsvtav1-command (file)
  ""
  :man-page "ffmpeg"
  :value '("-crf 32" "-preset 4" "-c:a libopus")
  ["Infile"
   ("-i" "input" "-i "
    :class transient-option
    :prompt "Input file: "
  ...
  ["Outfile"
   ("-o" "output" ""
    ...
    :reader my/transient-output-reader
    :always-read t
    :init-value (lambda (obj) (oset obj value (my/transient-output-reader nil "${f%.*}_av1.mkv"))))]
  [my/transient-common-menu
   [(my/shell-command-action
     :command-format
     "ffmpeg %i -c:v libsvtav1 -map_metadata 0 %O %o"
     :command-format-spec ((?i . (my/transient-infix-values '("-i")))
                           (?O . (my/transient-infix-values '("-c" "-p" "a" "-B")))
                           (?o . (my/transient-infix-values '("-o")))))]]
  (interactive (list (my/determine-file nil)))
...
(transient-define-prefix my/handbrakecli-command (file)
  ...
  :value '("--markers" "--main-feature" "--all-subtitles"
           "--encoder x264" "--encoder-preset fast" "--quality 18.0"
  ...
  ["Destination Options"
   ("-o" "Destination file name" ("-o " "--output ")
    ...
    :reader my/transient-output-reader
    :always-read t
    :init-value (lambda (obj) (oset obj value (my/transient-output-reader nil "${f%.*}.mp4"))))
...
(transient-define-prefix my/dvdbackup-command ()
  ""
  :man-page "dvdbackup"
  :value '("--input=/dev/sr0" "--output=dvd-out")
...
(transient-define-prefix my/arbitrary-shell-command (command)
  ...
  [("c" "shell command" ""
    ...
    :reader my/transient-read-shell-command)
   ("o" "option strings" ""
    :class transient-option)]
  [(my/shell-command-action
    :command-format "%a")]
  (interactive (list (my/transient-read-shell-command)))
  (transient-setup 'my/arbitrary-shell-command))

(transient-define-prefix my/rtcwake-command (&optional number)
  ""
  :man-page "rtcwake"
  :value `(,(concat "--date " (my/transient-read-time nil "+6h45m" nil)))
...
(defun my/good-night-command ()
  ...
  (setq my/shell-command-force-action "M-i")
  (with-current-buffer (get-buffer-create my/shell-command-buffer)
    (erase-buffer))
...
(defun my/transient-next ()
  ...
  (setq my/good-night-loop-count
        (if (< my/good-night-loop-count 3)
          ...
          (with-current-buffer my/shell-command-buffer
            (mark-whole-buffer)
            (detached-shell-command
             (buffer-substring-no-properties (region-beginning) (region-end)))
            (setq my/shell-command-force-action nil)
            ;; (kill-this-buffer)
            )
          0)))
#+end_src
...
(transient-define-prefix my/pandoc-command (file)
  ...
  ["Options"
   ...
   ("-o" "output file" "--output="
    :class transient-option
    :reader my/transient-output-reader
    :init-value (lambda (obj) (oset obj value (my/transient-output-reader nil "${f%.*}.org" nil))))]
  ["Inputs"
   ("f" "file" ""
    :class transient-option
    :prompt "Input file: "
...
(transient-define-prefix my/yt-dlp-command (url &optional output duration-p play-p)
  ...
  ["Options"
   ...
   ("-d" "duration" "--get-duration --no-warnings"
    p    :init-value (lambda (obj) (oset obj value (and duration-p "--get-duration --no-warnings"))))
   ("-f" "format" "--format "
    :class transient-option)
