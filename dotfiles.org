...
- Back-up Cinnamon Desktop Configuration
  #+begin_src shell :eval no :noweb yes :tangle no :noweb-ref morning-batch
  ...
  dconf load /org/cinnamon/ < <<home-dir()>>/.local/state/cinnamon/desktop.conf
  #+end_src
  ...
  (list
   ...
   (service home-dotfiles-service-type
            (home-dotfiles-configuration
             (layout 'plain)
             (source-directory "<<ghq-proj-path(name="p-snow/environment")>>")
             (directories '("./home")))))))
#+end_src
...
(setf tangle/system (expand-file-name "system" tangle/repo)
      tangle/home (expand-file-name "home" tangle/repo)
      tangle/config-dir (expand-file-name ".config" tangle/home)
...
(setf tangle/dedicated-elisp-dir
      (expand-file-name "lisp/dedicated" tangle/user-emacs-directory)
      tangle/dedicated-template-dir
      (expand-file-name "tempel/templates/dedicated" tangle/emacs-etc-dir))
#+end_src
...
PLOCATE_DB=~/.local/share/plocate/plocate.db
if [ ! -d $(dirname $PLOCATE_DB) ]; then
  mkdir -p $(dirname $PLOCATE_DB)
fi
...
$GUIX_EXTRA_PROFILES/base/base/sbin/updatedb -l 0 -U <<home-dir()>> -e <<htmlize-dir()>> -e <<trash-dir()>> -o <<plocate-db()>>
#+end_src
...
ghq list --full-path $name | head -n 1
#+end_src
...
TORRENT_DIR=~/.local/share/rtorrent/files
# if [ ! -d $TORRENT_DIR ]; then
#   mkdir -p $TORRENT_DIR
# fi

realpath $TORRENT_DIR
#+end_src
