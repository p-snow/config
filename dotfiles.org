...
#+begin_src emacs-lisp
  (use-package org-agenda
    :after (org org-ql my/org-archive-datetree)
...
#+begin_src emacs-lisp
  (use-package org-capture
    :after (org my/org-archive-datetree)
    :hook
...
#+begin_src emacs-lisp
  ...
  (defun my/english-p (string)
    "Return t if `STRING' is probably English."
    (string-match-p
     (format "\\`[%s]+\\'" "[:ascii:]’“”–")
     string))
#+end_src
...
#+begin_src emacs-lisp :tangle (expand-tangle-target ".emacs.d/site-lisp/org-relate.el")
  ...
  (add-hook 'my/org-archive-before-archive-hook
...
#+begin_src emacs-lisp :tangle (expand-tangle-target ".emacs.d/site-lisp/org-english.el")
  (defvar my/org-english-file (file-name-concat org-directory "note" "english.org")
  ...
  (defun my/org-english-insert-japanese-pronunciation (w)
    ...
    (let (pron)
      (request
        (concat "https://eow.alc.co.jp/search?q=" (downcase w))
        :sync t
        :parser
        (lambda ()
                ...
                (when (string-match
                       (rx (seq (opt "【レベル】" (** 1 2 digit) "、")
                                ...
                                (+ (or (category combining-diacritic)
                                       ...
                                       (category korean)))
                                "、"
                                (? (or (seq "【＠】" (opt "[US]") (+ (category japanese-katakana-two-byte)) (? "、"))
                                       (seq (+ any) "*")))))
                       str)
                  (match-string 0 str))))))
        :success
