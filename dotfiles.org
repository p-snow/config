...
(defun elfeed-search-show-media-duration ()
  ...
  (let* ((entry (elfeed-search-selected :single))
    ...
    (let ((my/execute-command-function '(shell-command-to-string string-trim princ)))
      (dispatch-yt-dlp-dwim url nil t)
      (dispatch-execute-macro))))
#+end_src
...
**** dispatch-dwim based on transient
:PROPERTIES:
...
(transient-define-suffix my/command-execute ()
...
#+NAME: config/bind-dwims
#+begin_src emacs-lisp
(defun my/execute-dispatch-dwim-command ()
  "Execute dispatch-*-dwim command."
  (declare (interactive-only command-execute))
  (interactive)
  (let ((read-extended-command-predicate
         (lambda (sym buf)
           (string-match-p (rx "dispatch" (+ nonl) "dwim" eos)
                           (symbol-name sym)))))
    (execute-extended-command current-prefix-arg)))
(keymap-global-set "H-D" 'my/execute-dispatch-dwim-command)
#+end_src
...
#+begin_src emacs-lisp
(transient-define-prefix dispatch-mpv-dwim (&optional file no-resume start end)
  ""
  :man-page "mpv"
  ...
  (interactive)
  (transient-setup 'dispatch-mpv-dwim nil nil
    ...
    (mapc (lambda (map)
            (lambda-key map "P"
                        (lambda (link)
                          ...
                          (dispatch-mpv-dwim (my/flink-path link) t))))
    ...
    (mapc
     (lambda (map)
       (lambda-key map "P"
                   (lambda (list)
                     "play media in LIST"
                     (dispatch-mpv-dwim (mapcar (lambda (item)
...
#+begin_src emacs-lisp
(transient-define-prefix dispatch-ffmpeg-opus-dwim (&optional file)
  ""
  ...
  [(my/command-execute
    ...
    (format "parallel ffmpeg -i '{}' -c:a libopus -ac 2 %%a '{.}'.opus ::: %s"
            (plist-get (oref transient-current-prefix scope) 'file)))]
  (interactive)
  (transient-setup 'dispatch-ffmpeg-opus-dwim nil nil
                   :scope (plist-put nil 'file (dispatch-command-dwim/determine-file file))))
#+end_src
...
#+begin_src emacs-lisp
(transient-define-prefix dispatch-sleep-rtcwake-dwim (&optional number)
  ""
  :man-page "rtcwake"
  [("-b" "time before sleep" "" :prompt "amount of time [s/m/h/d]: "
    :class transient-option :always-read t :allow-empty nil
    :init-value (lambda (obj) (oset obj value (or number "30m"))))
   ...
   ("-j" "wake-up job" "" :prompt "Wake-up job: " :class transient-option
  ...
  [(my/command-execute
    :shell-command-string "sleep %n; pwsudo rtcwake -l -m mem -v %d; %j"
    :format-spec ((?n . (dispatch-shell-command--key-vals '("-b")))
                  (?d . (dispatch-shell-command--key-vals '("-t")))
                  (?j . (dispatch-shell-command--key-vals '("-j")))))]
  (interactive)
  (transient-setup 'dispatch-sleep-rtcwake-dwim))
...
(defun my/transient-read-time (prompt default-time history)
  ...
  (or (seq-some (lambda (d)
                  (let ((time-str (string-trim
                    ...
                    (if (or (and (string-prefix-p "date: invalid date"
                                                  time-str)
                                 (message "Input time is invalid"))
                            (and (time-less-p (time-add nil (seconds-to-time (* 12 60 60)))
                                              (encode-time (parse-time-string time-str)))
                                 ;; confirm the time that is more than 12 hours later
...
#+name: pandoc-dwim
#+begin_src emacs-lisp
(transient-define-prefix dispatch-pandoc-dwim (&optional file)
  ""
  [(my/pandoc-to)]
  [(my/command-execute
    :shell-command-string
    (format "parallel pandoc %%a '{}' ::: %s"
            (plist-get (oref transient-current-prefix scope) 'file)))]
  (interactive)
  (transient-setup 'dispatch-pandoc-dwim nil nil
                   :scope (plist-put nil 'file (dispatch-command-dwim/determine-file file))))
...
#+name: yt-dlp-dwim
#+begin_src emacs-lisp :noweb tangle
(transient-define-prefix dispatch-yt-dlp-dwim (&optional url output duration-p play-p)
  ""
  :man-page "yt-dlp"
  ...
  (interactive)
  (transient-setup 'dispatch-yt-dlp-dwim nil nil
...
#+begin_src emacs-lisp
(transient-define-prefix dispatch-du-dwim (&optional file)
  "Dispatch du command."
  :man-page "du"
  ...
  [(my/command-execute
    ...
    (format "ionice -c2 -n7 nice -n19 du %%a %s"
            (plist-get (oref transient-current-prefix scope) 'file)))]
  (interactive)
  (transient-setup 'dispatch-du-dwim nil nil
                   :scope (plist-put nil 'file (dispatch-command-dwim/determine-file file))))
#+end_src
...
emacsclient -s server-<<hash-string(seed="server", len=6)>> --eval "(progn (dispatch-yt-dlp-dwim \"$(xclip -out)\"${arg})(dispatch-execute-macro))"
#+end_src
