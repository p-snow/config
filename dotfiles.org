  ...
  (defun my/lfile-completing-read (&optional arg)
    ...
    (let* ((paths (split-string
                   (shell-command-to-string
                    (format "locate -q -r ^%s" (shell-quote-argument (expand-file-name my/lfile-root))))
                   "\n" t))
...
- updatedb cron job
  ...
  #+begin_src conf :noweb yes :noweb-ref cronjob-pc
    30 4 * * * /bin/bash -ci "[[ -d $(dirname <<locate-db()>>) ]] || mkdir -p $(dirnmae <<locate-db()>>) && which updatedb | xargs -I {} pwsudo {} -U ~/ -l 0 -o <<locate-db()>>"
  #+end_src
