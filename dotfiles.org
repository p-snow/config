...
- Automating Script
  #+begin_src conf :noweb yes :noweb-ref cronjob-pc
    <<hash-to-num(s="upch",max_n=59)>> <<night-time(lag=4)>> * * * /bin/bash -ci "update-channels && activate-profiles && guix gc -d 2m -F 20G >> $LOG_DIR/guix-update.log 2>&1"
  #+end_src
...
#+name: night-time
#+header: :var lag=0 bedin-time=23
...
#+begin_src conf :noweb yes :noweb-ref cronjob-pc
  <<hash-to-num(s="updb",max_n=59)>> <<night-time(lag=3)>> * * * /bin/bash -ci "updatedb-home >> $LOG_DIR/updatedb-home.log 2>&1"
#+end_src
...
#+begin_src conf :noweb yes :noweb-ref cronjob-pc
  20 0 * * * /bin/bash -ci "skkdic_uniq.sh >> $LOG_DIR/skkdic_uniq.log 2>&1"
#+end_src
