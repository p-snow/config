...
export GUIX_LOCPATH="${GUIX_EXTRA_PROFILES}/base/base/lib/locale"
...
*** gnu/packages
:PROPERTIES:
:CREATED:  [2024-04-03 Wed 16:08]
:END:

**** base.scm
:PROPERTIES:
:CREATED:  [2024-04-03 Wed 16:10]
:END:
...
*** basic configuration
:PROPERTIES:
:header-args+: :tangle (expand-file-name "my-config.el" tangle/emacs-config-dir) :tangle-mode (identity #o644)
:CREATED:  [2023-01-20 Fri 10:22]
:END:
...
(use-package consult-ghq
  ...
  :defer t
  :after consult
  :bind (("C-c q f" . consult-ghq-find)
         ("C-c q g" . consult-ghq-grep))
  :custom
...
:header-args+: :tangle (expand-file-name "elisp-devel.el" tangle/emacs-config-dir)
:END:
...
(use-package define-word
  ...
  (defun my/define-word (word)
    ...
    (interactive (list (if (use-region-p)
                         ...
                         (read-string "Word: " (word-at-point t) 'my/define-word-minibuffer-history))))
...
(no-littering-expand-var-file-name "htmlize")
#+end_src

#+begin_src conf :noweb yes :noweb-ref tmpfiles-cleanup :tangle no
d <<htmlize-dir()>> 0755 - - 5d
#+end_src
...
#+begin_src bash :noweb tangle :tangle (expand-file-name "emacs-everywhere" tangle/local-bin-dir)
...
#+begin_src emacs-lisp :comments no :tangle (expand-file-name "org-babel-config.el" tangle/proj-dir) :noweb-ref babel-conf
(with-eval-after-load 'ob-core
...
(transient-define-prefix rtcwake-dispatch-dwim (&optional number)
  ...
  [("-t" "wake-up time" "--date " :prompt "Wake-up time: " :class transient-option
    ...
    :init-value (lambda (obj) (oset obj value (my/transient-read-time nil "+6h40m" nil))))]
...
(transient-define-prefix yt-dlp-dispatch-dwim (&optional url output duration-p play-p)
  ...
  [(dispatch-dwim-execute
    ...
    :command-format-spec ((?u . (plist-get (oref transient-current-prefix scope) 'url))))]
  (interactive)
  (setq my/detached-bury-buffer t)
  ...
  :bind (:map my/org-global-map
              ...
              ("j" . (lambda (word)
                       ...
                       (interactive (list (cond
                                           ((region-active-p)
                                            (buffer-substring-no-properties (region-beginning) (region-end)))
                                           (t (read-string "Word: " (word-at-point t))))))
...
Capture URL or file currently visiting as well as a sentence at point."
  (interactive (list (if (region-active-p)
                         (buffer-substring-no-properties (region-beginning) (region-end))
                       (read-string "Word to capture: " (word-at-point t)))))
...
mkdir -p $(dirname <<plocate-db()>>) && \
  $GUIX_EXTRA_PROFILES/base/base/sbin/updatedb -l 0 -U <<home-dir()>> -e <<htmlize-dir()>> -o <<plocate-db()>> \
                                               <<updatedb-opt>>
#+end_src
...
#+begin_src conf :noweb yes :noweb-ref cronjob-user
40 */4 * * * <<which(filename="bash")>> -ci "sync-annex-doc 2>&1 | tee -a <<log-dir()>>/sync-git-annex.log"
#+end_src

#+begin_src bash :noweb tangle :tangle (expand-file-name "sync-annex-doc" tangle/local-bin-dir)
(cd /mnt/exdoc/doc/ 2>/dev/null && git rev-parse --is-inside-work-tree 2>/dev/null && git annex sync && git annex get)
(cd /mnt/vt-m/doc/ 2>/dev/null && git rev-parse --is-inside-work-tree 2>/dev/null && git annex sync && git annex get)
(cd /mnt/vt-h/doc/ 2>/dev/null && git rev-parse --is-inside-work-tree 2>/dev/null && git annex sync && git annex get)
(cd /mnt/vt-j/doc/ 2>/dev/null && git rev-parse --is-inside-work-tree 2>/dev/null && git annex sync && git annex get)
(cd <<doc-dir()>> 2>/dev/null && git rev-parse --is-inside-work-tree 2>/dev/null && git annex sync)

(cd /mnt/exdoc/doc/ 2>/dev/null && git rev-parse --is-inside-work-tree 2>/dev/null && git annex sync)
#+end_src
...
ghq list --full-path $projname | head -n 1
#+end_src
...
The Elisp variable below is used in tangling from my personal org repository.

#+begin_src emacs-lisp :noweb yes :noweb-ref elisp-code
(defvar dotfiles/tangled-dir
  (file-name-concat
   "<<ghq-proj-path(projname="p-snow/dotfiles")>>"
   ".config/emacs/lisp/tangled"))
#+end_src
