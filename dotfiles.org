...
(with-eval-after-load 'embark
  (define-key embark-region-map "\"" #'my/tts-say-it))
...
(defun my/tts-say-it (text)
  ...
  (interactive (list (cond
                      ...
                      (sentence-at-point))))
  (tts-openai-request
   (cdr (assoc "API key" (auth-source-pass-parse-entry "openai.com")))
   text
   #'my/tts-play))
...
**** Creating Links betweeb Project-Task Items

#+begin_src emacs-lisp
(defun my/insert-project-link ()
  "Insert drawer links by `org-super-links' indicating project-task relationship."
  (interactive)
  (when (and (require 'org-super-links nil t)
             (org-get-todo-state))
    (let ((org-super-links-link-postfix " <project>")
          (org-super-links-backlink-postfix " <project>\n")
          (org-refile-targets `((nil :tag . "project")
                                (nil :tag . "refile"))))
      (org-super-links-quick-insert-drawer-link))))

(with-eval-after-load 'org-refile
  (add-hook 'org-after-refile-insert-hook
            #'my/insert-project-link))
#+end_src
...
(defun my/org-english-link ()
  "Make a link as well as a backlink from current word to the target with context."
  (interactive)
  (when org-tidy-mode
    (org-tidy-untidy-buffer))
  (let* ((contexts '("synonym" "antonym"
                     "distinct" "similar" "akin" "relevant"
                     "morphological" "derivative"))
         (context (completing-read "Context: " contexts))
         (org-super-links-link-postfix (format " <%s>" context))
         (org-super-links-backlink-postfix (format " <%s>\n" context)))
    (org-super-links-quick-insert-drawer-link)))
...
(defun dispatch-command-dwim/determine-file (&optional file)
  "This is obsolete. Use `dispatch-dwim-determine-file-name'."
  (cl-labels ((read-file ()
                (read-file-name "File: ")))
    (apply (or (and file
...
(defmacro dispatch-dwim-determine-file-name ()
  ...
  `(cond
    ...
    (t (when-let ((f (ffap-file-at-point)))
         (when (file-exists-p f)
           (dispatch-dwim--paths-string f))))))
...
$GUIX_EXTRA_PROFILES/base/base/sbin/updatedb -l 0 -U <<home-dir()>> --prune-bind-mounts 0 -e <<htmlize-dir()>> -e <<trash-dir()>> -o <<plocate-db()>>
#+end_src
