...
#+begin_src emacs-lisp
  ...
  (use-package ddskk
    ...
    (skk-get-jisyo-directory (expand-file-name "get-jisyo" skk-jisyo-dir))
    (skk-jisyo `(,(expand-file-name "jisyo" skk-jisyo-dotemacs-dir) . utf-8))
    ...
    (skk-extra-jisyo-file-list
     (seq-remove (lambda (dic)
                 ...
                 (append (file-expand-wildcards (expand-file-name "SKK-JISYO.*" skk-get-jisyo-directory))
                         (file-expand-wildcards (expand-file-name "open-jisyo/SKK-JISYO.*" skk-jisyo-dir)))))
    ;; jisyo server
...
#+begin_src shell :tangle (expand-tangle-target ".local/bin/skkdic_uniq.sh") :shebang #!/usr/bin/env bash :tangle-mode (identity #o755)
  tmp_euc=$(mktemp)
  tmp_utf8=$(mktemp)
  dic_orig=~/.emacs.d/ddskk/jisyo
  dic_uniq=~/.emacs.d/ddskk/jisyo.uniq

  skkdic-expr2 ~/share/skk/get-jisyo/SKK-JISYO.* > ${tmp_euc}
