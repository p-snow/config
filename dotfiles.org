...
#+begin_src conf :noweb yes :noweb-ref cronjob-pc
  20 * * * * /bin/bash -ci "pwsudo btrbk --config ~/.config/btrbk/btrbk.conf --loglevel error snapshot >> $LOG_DIR/btrbk.log 2>&1"
#+end_src
...
#+begin_src conf :noweb tangle :tangle (expand-tangle-target ".config/btrbk/btrbk.conf")
  ...
  volume /mnt/ssd01
    ...
    subvolume incumbents/doc
      # current 8 weekly and 12 monthly snapshots are preserved
      snapshot_preserve 8w 12m
    subvolume incumbents/share
      snapshot_preserve 72h 60d 12m
  ...
  volume /mnt/nvme01
    ...
    subvolume incumbents/media
      # only last snapshot will be preserved
...
- a script encoding DVD data to H.265 video
  ...
  #+begin_src shell :tangle (expand-tangle-target ".local/bin/dvdpack")
    #!/usr/bin/env bash
    set -euo pipefail
...
- a script encoding to VP9 video
  ...
  #+begin_src shell :tangle (expand-tangle-target ".local/bin/vp9encode")
    #!/usr/bin/env bash
    set -euo pipefail
