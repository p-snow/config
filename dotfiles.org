...
#+begin_src emacs-lisp :tangle (expand-file-name "my-org-config.el" my/user-config-dir)
  (use-package org-web-track
    :after org
    :bind (("C-o W W" . my/org-web-track-list)
...
#+begin_src emacs-lisp :tangle (expand-file-name "org-web-track.el" my/user-package-dir)
  ...
  (defun org-web-track-value (url &optional sync on-success &rest args)
    ""
    (let ((tracker (assoc-default url org-web-track-trackers #'string-match))
  ...
  (defun org-web-track--do-update (value &rest args)
    ""
    (let* ((marker (car args))
           (last-date (org-entry-get marker "TRACK_DATE"))
           (value-last (org-entry-get marker "TRACK_VALUE")))
      (org-entry-put marker "TRACK_VALUE" value)
      (org-entry-put marker "TRACK_DATE" (format-time-string (org-time-stamp-format t t)))
      (when (or (not value-last)
                (and (stringp value-last)
                     (not (string= value value-last))))
        (and (stringp value-last)
             (org-entry-put marker
                            org-web-track--propname-last-value
                            value-last))
        (org-entry-put marker "TRACK_VALUE" value)
        ;; FIXME: value history in log book
        ;; (setq-local org-log-note-headings
        ;;             (append (default-value 'org-log-note-headings)
        ;;                     `((web-track . ,(concat (format "Value %-7s" value)
        ;;                                             (if value-last
        ;;                                                 (format "from %-7s" value-last)
        ;;                                               (make-string 13 ? ))
        ;;                                             " on %t")))))
        ;; (org-add-log-setup 'web-track nil nil 'time)
        (message "Tracking value was updated."))))
  ...
  (defun org-agenda-web-track-columns ()
    ...
    (when (eq major-mode 'org-agenda-mode)
      (let* ((org-overriding-columns-format
              (format "%%20ITEM %%7TRACK_VALUE(VALUE) %%7%s(LAST) %%TRACK_DATE(DATE)"
                      org-web-track--propname-last-value)))
        (org-agenda-columns))))
  ...
  (let ((request-backend 'url-retrieve)
    ...
    (mapc (lambda (price-entry)
            (let* ((heading (org-element-property :raw-value price-entry))
                   ...
                   (cmpr (lambda (value &rest args)
                           (let ((last (car args)))
                             (unless (string= value last)
                               (setq mail-msg
                                     (concat mail-msg
                                             (format "\"%s\": %s => %s\n"
                                                     heading last value))))))))
