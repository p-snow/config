...
**** Org-mode
:PROPERTIES:
:header-args+: :tangle (expand-file-name ".emacs.d/lisp/my-org.el")
:END:
...
#+begin_src emacs-lisp
  ...
  (bind-keys ("C-o b i" . my/org-babel-lob-ingest-batch))
  ...
  (defun my/org-reset-dwim ()
    "Exert reset procedure depending on org context at point."
    (interactive)
    (cond
  ...
  (bind-keys ("C-o SPC" . my/org-reset-dwim))
  ...
  (defun my/org-alter-heading ()
    "Interpolate an org heading of the entry at point."
    (interactive)
...
#+begin_src emacs-lisp
  (let ((map org-mode-map))
    ...
    (define-key map (kbd "C-c D v") #'my/org-download-video-link-at-point)
    (define-key map (kbd "C-c D a") #'my/org-download-audio-link-at-point)
    (define-key map (kbd "C-c D =") #'my/org-show-media-duration-at-point)
...
*** further extension
:PROPERTIES:
:header-args+: :tangle (expand-file-name "my-extension.el" my/user-config-dir)
:CREATED:  [2023-01-20 Fri 10:22]
:END:
...
#+begin_src emacs-lisp
  (defun my/review-tasks (resume-p)
...
#+begin_src emacs-lisp :tangle (expand-file-name "my-config.el" my/user-config-dir)
  (use-package org-web-track
    :bind (("C-o w u" . org-web-track-update)
           ("C-o w v" . org-web-track-view)
           :map org-agenda-mode-map
...
#+begin_src emacs-lisp :tangle (expand-file-name "org-web-track.el" my/user-package-dir)
  (defvar org-web-track-trackers
    '(("www\.amazon\.co\.jp" . org-web-track-tracker--amazon-jp))
  ...
  (defun org-web-track-update ()
    "Update tracking data for org entry at point."
    (interactive)
    (let ((url (org-entry-get (point) "TRACK_URL"))
          (last-date (org-entry-get (point) "TRACK_DATE"))
          (last-val (org-entry-get (point) "TRACK_VALUE")))
      (unless (or (and (not url)
                       (user-error "TRACK_URL is missing"))
                  (and last-date
                       (not (time-less-p (time-add
                                          (encode-time (org-parse-time-string last-date))
                                          (days-to-time 1))
                                         (encode-time (parse-time-string (current-time-string)))))
                       (user-error "Not elapsed 1 day since last update")))
        ;; check if URL is available
        (and (not (= (call-process-shell-command
                      (format "curl -o /dev/null --connect-timeout 1 -lsf \"%s\"" url))
                     0))
             (user-error "URL is not accessible"))
        (and (not (setq-local val (funcall (assoc-default url org-web-track-trackers #'string-match)
                                           url)))
             (user-error "Could not retrieve valid value"))
        (org-entry-put (point) "TRACK_DATE" (format-time-string (org-time-stamp-format t t)))
        (when (or (not last-val)
                  (and (stringp last-val)
                       (not (string= val last-val))))
          (and (stringp last-val)
               (org-entry-put (point) "TRACK_VALUE_PREV" last-val))
          (org-entry-put (point) "TRACK_VALUE" val)
          (setq-local org-log-note-headings
                      (append (default-value 'org-log-note-headings)
                              `((web-track . ,(concat (format "Value %-7s" val)
                                                      (if last-val
                                                          (format "from %-7s" last-val)
                                                        (make-string 13 ? ))
                                                      " on %t")))))
          (org-add-log-setup 'web-track nil nil 'time)))))

  (defun org-agenda-web-track-update ()
    "Invoke `org-web-track-update' in org agenda mode."
    (interactive)
    (org-agenda-archive-with #'org-web-track-update))

  (defun org-web-track-tracker--amazon-jp (url)
    "A tracker responsible for retrieving price data in URL."
    (concat (seq-mapcat (lambda (char)
                          (string-replace "\\" "¥"
                                          (string (japanese-hankaku char))))
                        (string-trim (shell-command-to-string
                                      (format "curl --silent -A 'Mozilla/5.0' \"%s\" \\
    \| pup 'span[id=\"priceblock_ourprice\"],span[id=\"kindle-price\"],span[class=\"a-text-price\"],span[class=\"a-offscreen\"] json{}' \\
    \| jq -r '.[0].text'"
                                              url))))))

  (defun org-web-track-view ()
    ...
    (let ((org-agenda-files
      ...
      (when (require 'org-ql nil t)
        (org-ql-search (org-agenda-files) '(property "TRACK_URL")))))

  (defun org-agenda-web-track-columns ()
    "Display column view showing TRACK_VALUE,TRACK_VALUE_PREV and TRACK_DATE property in org agenda view."
    (interactive)
    (when (eq major-mode 'org-agenda-mode)
      (let* ((org-agenda-overriding-columns-format "%20ITEM %7TRACK_VALUE(VALUE) %7TRACK_VALUE_PREV(PREV) %TRACK_DATE(DATE)"))
        (org-agenda-columns))))
...
#+begin_src emacs-lisp :tangle (expand-file-name "org-web-track-mail.el" my/user-batch-dir)
  (require 'org)
  (require 'org-ql)
  (require 'text-mail)
  ...
  (let (mail-msg)
    (mapc (lambda (price-entry)
            (let* ((heading (org-element-property :raw-value price-entry))
                   ...
                   (value-last (org-element-property :TRACK_VALUE price-entry))
                   (value (funcall (assoc-default track-url org-web-track-trackers #'string-match)
                                   track-url)))
              (unless (string= value value-last)
                (setq mail-msg
                      (concat mail-msg
                              (format "\"%s\": %s => %s\n"
                                      heading value-last value))))))
          (org-ql-select
            (directory-files-recursively org-directory
                                         (rx (seq ".org" eol)))
            `(and (property ,"TRACK_URL")
...
***** unit test for my/org-price-checker-retrievers
:PROPERTIES:
:CREATED:  [2023-01-07 Sat 20:36]
:END:
...
#+begin_src emacs-lisp  :tangle (expand-file-name "org-english.el" my/user-package-dir)
  (require 'define-word)
