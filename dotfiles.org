...
:header-args+: :tangle (expand-file-name "my-org-config.el" my/user-config-dir)
:END:
***** set timer when clock in for a task
  ...
  With `C-u' prefix ARG, try to browse in Emacs."
    ...
    (let* ((heading (org-get-heading t t t t))
           ...
           (browse-url-browser-function
            (cond
             ((equal arg '(16)) 'browse-url-xdg-open)
             ((equal arg '(4)) 'browse-url-emacs)
             (t 'eww-browse-url))))
...
***** pad void time in org clock            :ARCHIVE:
:PROPERTIES:
