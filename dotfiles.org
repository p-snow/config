...
(transient-define-prefix rtcwake-dispatch-dwim (&optional number)
  ...
  [("(" "preceding sleep" dispatch-dwim-sibling-command
    :prefix sleep-dispatch-dwim :prefix-arg ("30m") :priority 20
    :default "sleep 30m")
   (")" "following command" dispatch-dwim-sibling-command
    :prefix option-dispatch-dwim :prefix-arg ("morning.sh" "which system.sh | xargs pwsudo; morning.sh; weekend.sh")
    :priority 90 :default "morning.sh")
   (dispatch-dwim-execute
    :shell-command-string "pwsudo rtcwake -l -m mem -v %d"
...
#+begin_src emacs-lisp :comments no :tangle (expand-file-name "dispatch-dwim.el" tangle/emacs-sitelisp-dir)
;; -*- lexical-binding: t -*-
...
(defclass dispatch-shell-command (transient-suffix)
  ((shell-command-string :initarg :shell-command-string :initform "%a")
...
(cl-defmethod transient-init-value ((obj dispatch-shell-command))
  (unless (slot-boundp obj :shell-command-string)
    (oset obj shell-command-string "%a"))
                     ...
                     (append
                      ...
                      (when format-spec
                        (mapcar (lambda (key-val) (cons (car key-val) (eval (cdr key-val))))
                                format-spec)))))
         (comm (mapconcat (lambda (elm) (cadr elm))
...
(defclass dispatch-dwim-sibling-suffix (transient-suffix)
...
(transient-define-suffix dispatch-dwim-sibling-command ()
  :class dispatch-dwim-sibling-suffix
...
(cl-defmethod transient-init-value ((obj dispatch-dwim-sibling-suffix))
  (when (slot-boundp obj :default)
    (setf (alist-get (oref obj key) dispatch-dwim--precedings-followings)
          (list (oref obj default)
                (oref obj priority)))))

(cl-defmethod transient-format-value ((obj dispatch-dwim-sibling-suffix))
...
(cl-defmethod transient-format ((obj dispatch-dwim-sibling-suffix))
