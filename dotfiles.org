...
#+begin_src conf :noweb tangle :tangle (expand-file-name ".config/systemd/system/xremap.service")
  ...
  [Service]
  ExecStart=<<which(filename="xremap")>> <<conf-dir()>>/xremap/config.yml
  Restart=always
  Type=simple
...
#+begin_src emacs-lisp :tangle no :noweb-ref additional-elisp
  (use-package emacs
    :no-require t
    ;; :defer 5
    ;; :config
    ;; (call-process-shell-command "start-xkb")
    :hook
    (after-init . (lambda ()
                    (message "Calling xkb...")
                    (shell-command "start-xkb")))
    )
#+end_src
