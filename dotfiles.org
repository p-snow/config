...
:ID:       6757cb6b-289c-4b4d-8c47-76b03681cdfe
:END:

- which: return absolute path for file using which command
  #+name: which
  #+begin_src bash :var filename=""
    which ${filename}
  #+end_src
- random: return random number between 0 and max
  #+name: random
  #+header: :var max=10
...
- Automating Script
  #+begin_src conf :noweb yes :noweb-ref cronjob-pc
    <<random(max=59)>> <<night-time(lag=4)>> * * * /bin/bash -ci "update-channels && activate-profiles && guix gc -d 2m -F 20G >> $LOG_DIR/guix-update.log 2>&1"
  #+end_src
...
#+begin_src emacs-lisp
  (use-package org-web-track
    ...
    (defun my/org-web-track-mail-chnages ()
      "docstring"
      (when-let* ((changes (org-web-track-update-all t))
                  (mail-msg (mapconcat
                             (lambda (chg)
        ...
        (let ((message-kill-buffer-on-exit t))
          (message-mail user-mail-address "Web Track Results")
          (message-goto-body)
          (insert mail-msg)
...
#+begin_src shell :noweb tangle :tangle (expand-file-name ".local/bin/updatedb-home") :tangle-mode (identity #o755) :shebang #!/usr/bin/env bash
  . ~/.bashrc
...
#+begin_src conf :noweb yes :noweb-ref cronjob-pc
  <<random(max=59)>> <<night-time(lag=3)>> * * * /bin/bash -ci "updatedb-home >> $LOG_DIR/updatedb-home.log 2>&1"
#+end_src
...
#+begin_src scheme :noweb-ref guix-creative
  "slop"
  "gimp"
  "obs"
  "kdenlive"
