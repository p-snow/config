...
#+begin_src emacs-lisp
  ...
  (setq package-user-dir (expand-file-name "elpa" user-emacs-directory-default))
#+end_src
...
**** basic config
:PROPERTIES:
:header-args+: :tangle (expand-tangle-target ".emacs.d/lisp/01-basic.el")
:END:
...
Disclaimers: There are some face definitions which use an extra large font since I am [[https://en.wikipedia.org/wiki/Visual_impairment][visually impaired]].
...
#+begin_src emacs-lisp
  ;; Don't show line and column number in the mode line
  (line-number-mode -1)
  ...
  (defvar my/mode-line-buffer-name-length 15
...
#+begin_src emacs-lisp
  ...
  ;; search files in case insensitive way
...
This command was introduced at [[https://irreal.org/blog/?p=10318][here]] to insert comment box with fixed width based on [[help:fill-column][fill-column]].
...
**** built-in features
:PROPERTIES:
:header-args+: :tangle (expand-tangle-target ".emacs.d/lisp/02-built-ins.el")
:END:
***** Auto Revert
...
#+begin_src emacs-lisp
  (use-package Info
    ...
    (Info-additional-directory-list
     (append `(,(expand-file-name ".local/share/info" "~"))))
    :config
    (require 'info-look)
...
#+begin_src emacs-lisp
  (defvar skk-jisyo-dir (expand-file-name "~/share/skk"))
  (defvar skk-jisyo-dotemacs-dir (expand-file-name "ddskk" user-emacs-directory-default))
  ...
  (use-package ddskk
    ...
    (skk-init-file "~/.emacs.d/ddskk/init.el")
    ...
    ;; config file
    (skk-record-file (expand-file-name "record" skk-jisyo-dir))
    (skk-emacs-id-file (expand-file-name "emacs-id" skk-jisyo-dir))
    ;; jisyo
    (skk-share-private-jisyo t)
    (skk-compare-jisyo-size-when-saving nil)
    (skk-save-jisyo-instantly t)
    ;; jisyo file/directory
    (skk-jisyo (expand-file-name "jisyo" skk-jisyo-dotemacs-dir))
    (skk-backup-jisyo (expand-file-name "jisyo.bak" skk-jisyo-dir))
    (skk-large-jisyo (expand-file-name "large-jisyo" skk-jisyo-dir))
    ;; jisyo server
    (skk-server-host "localhost")
    (skk-server-portnum 1178)
    (skk-server-inhibit-startup-server t)
    ;; study
    (skk-study-file (expand-file-name "study" skk-jisyo-dir))
    (skk-study-backup-file (expand-file-name "study.bak" skk-jisyo-dir)))
#+end_src

***** skk-init-file

#+begin_src emacs-lisp :tangle (expand-tangle-target ".emacs.d/ddskk/init.el")
  ;; -*- mode:emacs-lisp; -*-
...

QnFq5wn2GR7s
=67og
-----END PGP MESSAGE-----
...
[[https://github.com/skeeto/elfeed][Elfeed]] is a powerful RSS feed reader with tag capability which is one of core software in my input workflow.
...

QGy4EBy3
=92VJ
-----END PGP MESSAGE-----
...
#+begin_src emacs-lisp
  (use-package ol
    ...
    :config
    ;; the file which is referred via ong link opens in same window
    ...
    ;; set 'sudo' link type to invoke command with root privilege
...
#+begin_src emacs-lisp
  ...
  (defvar org-readable-directory "~/tmp"
...
:PROPERTIES:
:header-args+: :tangle (expand-tangle-target ".emacs.d/lisp/prog.el")
:END:
  ...
  play it in media player."
    (cond
     ((or (string-match "\\([^/]+\\.pdf\\)" url)
      ...
      (let ((dl-file (expand-file-name (match-string 1 url)
...
#+begin_src emacs-lisp
  (defvar my/download-video-dir "~/data/media/video" "Directory where downloaded video locate.")
  ...
  (defvar my/download-audio-dir "~/data/media/audio" "Directory where downloaded audio locate.")
    ...
    (let* ((ipv4-addr (shell-command-to-string "hostname -I | cut -f1 -d' ' | tr -d '\n'"))
           ...
           (fmt-quality (cond
                         ...
                         ((or (string-prefix-p "192.168.100." ipv4-addr)
                              (member wifi-ssid '("saint_laurent_ap")))
...
#+begin_src emacs-lisp :tangle (expand-tangle-target ".emacs.d/site-lisp/org-english.el")
  ...
  Capture URL or file currently visiting as well as a sentence at point."
    ...
    (let* ((sentence-end-double-space nil)
           ...
           (sentence
            (seq-reduce (lambda (s w)
                          (replace-regexp-in-string w (format "[%s||%c]" w (string-to-char w))
                                                    s))
                        (split-string word " ")
                        (sentence-at-point))))
...
- xkeysnail.service
  #+begin_src conf :tangle (expand-tangle-target ".config/systemd/user/xkeysnail.service")
    ...
    [Service]
    KillMode=process
    ExecStart=/bin/bash -ic '%h/.local/bin/pwsudo %h/.guix-extra-profiles/desktop/desktop/bin/xkeysnail --watch --quiet %h/.config/xkeysnail/config.py'
    Type=simple
    Restart=always
  ...
  #+begin_src shell :tangle (expand-tangle-target ".local/bin/back_btrfs.sh")
    ...
    function backup_subvol() {
      ...
      if [[ "${src_back_last}" < "${snap_last}" ]]; then
        btrfs subv snap -r "${_src_snap_dir}/${snap_last}" "${_src_back_dir}/${src_back_last}"
      fi
      ...
      if [ -n "${parent}" ] && [ -n "${subvol}" ]; then
        ...
        pwsudo btrfs send -p "${_src_back_dir}/${parent}" "${_src_back_dir}/${subvol}" | sudo btrfs receive "${_dst_back_dir}"
      elif [ -z "${parent}" ] && [ -n "${subvol}" ]; then
        echo "Invoke full backup"
        pwsudo btrfs send "${_src_back_dir}/${subvol}" | sudo btrfs receive "${_dst_back_dir}"
      fi
