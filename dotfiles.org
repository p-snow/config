...
#+begin_src emacs-lisp :noweb tangle
(cl-command-defun my/play-media (file &key no-resume)
  "Build a shell command string to play media FILE.
...
during the previous playback."
  (format "mpv %s"
          (string-join
           (delq nil
                 (list (and no-resume "--no-resume-playback")
                       (cond
                        ((stringp file) (shell-quote-argument (expand-file-name file)))
                        ((listp file)
                         (mapconcat (lambda (f)
...
(with-eval-after-load 'embark
  (mapc (lambda (map)
          (lambda-key map "P"
                      (lambda (file)
                        "play media FILE"
                        (funcall-interactively 'my/play-media file)))
          (lambda-key map "M-P"
                      (lambda (file)
                        "play media FILE as ignoring resume playback"
                        (funcall-interactively 'my/play-media file :no-resume t))))
    ...
    (mapc (lambda (map)
            (lambda-key map "P"
                        (lambda (link)
                          ...
                          (funcall-interactively 'my/play-media (url-filename (url-generic-parse-url (org-link-expand-abbrev link))))))
            (lambda-key map "M-P"
                        (lambda (link)
...
#+begin_src emacs-lisp :tangle no :noweb-ref minibuffer-template
(play-media (my/play-media (read-file-name "File: ")))
(play-media-no-resume (my/play-media (read-file-name "File: ") :no-resume t))
#+end_src
