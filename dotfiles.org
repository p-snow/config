...
#+begin_src emacs-lisp
  ...
  (defun my/org-remove-priority-if-done ()
    ...
    (when (and
           ...
           (string-match-p
            (eval `(rx (or ,my/org-done-keyword--done
                           ,my/org-done-keyword--cancel
                           ,my/org-done-keyword--pending)))
            org-state)
  ...
  (defun my/org-remove-done-if-nonrepeated-habit ()
    ...
    (when (and (string-match-p my/org-done-keyword--done
                               org-state)
               (org-is-habit-p)
...
#+begin_src emacs-lisp
  ...
  (customize-set-variable 'org-id-link-to-org-use-id 'create-if-interactive)
  (with-eval-after-load 'org
...
***** sending org subtree contents via email
...
#+end_src
***** pad void time in org clock

#+begin_src emacs-lisp
  (with-eval-after-load 'org-clock
    (add-hook 'org-clock-out-hook #'my/org-clock-pad-void-time)
    (add-hook 'org-clock-cancel-hook #'my/org-clock-pad-void-time))

  (defun my/org-clock-pad-void-time ()
