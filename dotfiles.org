...
**** dispatch-shell-command based on transient
:PROPERTIES:
...
(defun my/determine-file-specifier (file)
  (cl-flet ((files-str (arg)
              (pcase arg
                ((and (and (pred stringp) (pred file-exists-p)) str)
                 (shell-quote-argument (expand-file-name str)))
                ((and (pred listp) li) (mapconcat 'files-str li " ")))))
    (let ((disp-comm (oref (transient-suffix-object) dispatch-command)))
      (cond
       ((equal disp-comm my/execute-command-function)
        (files-str file))
       ((equal disp-comm 'insert)
        (if (eq minibuffer-history-variable 'dired-shell-command-history)
            "*"
          (files-str file)))))))
#+end_src
...
#+begin_src emacs-lisp
(transient-define-prefix dispatch-mpv (file &optional no-resume start end)
  ""
  :man-page "mpv"
  ...
  [(my/command-execute
    ...
    (format "mpv %%a %s"
            (my/determine-file-specifier
             (plist-get (oref transient-current-prefix scope) 'file))))]
  (interactive "f")
  (transient-setup 'dispatch-mpv nil nil
  ...
  (with-eval-after-load 'org
    ...
    (mapc (lambda (map)
            (lambda-key map "P"
                        (lambda (link)
                          "play media file written in org LINK"
                          (dispatch-mpv (my/flink-path link) t))))
          (list embark-org-link-map))
    (mapc
     (lambda (map)
       (lambda-key map "P"
                   (lambda (list)
                     "play media in LIST"
                     (dispatch-mpv (mapcar (lambda (item)
                                             (when (string-match org-link-any-re (car item))
...
***** dispatch: ffmpeg opus
:PROPERTIES:
:CREATED:  [2023-11-12 Sun 22:00]
:END:

#+begin_src emacs-lisp
(transient-define-prefix my/ffmpeg-opus (file)
    ...
    (format "parallel ffmpeg -i '{}' -c:a libopus -ac 2 %%a '{.}'.opus ::: %s"
            (shell-quote-argument
             (expand-file-name
              (plist-get (oref transient-current-prefix scope) 'file)))))]
  (interactive "f")
  (transient-setup 'my/ffmpeg-opus nil nil
                   :scope (plist-put nil 'file file)))
#+end_src

***** dispatch: sleep-rtcwake
:PROPERTIES:
...
#+name: command/pandoc
#+begin_src emacs-lisp
(transient-define-prefix my/pandoc (file)
  ""
  [(my/pandoc-to)]
  [(my/command-execute
    :shell-command-string
    (format "parallel pandoc %%a '{}' ::: %s"
            (my/determine-file-specifier
             (plist-get (oref transient-current-prefix scope) 'file))))]
  (interactive "f")
  (transient-setup 'my/pandoc nil nil
                   :scope (when file (plist-put nil 'input file))))
