...
(transient-define-suffix my/shell-command-string (the-prefix-arg)
  :transient 'transient--do-exit
  :key "c"
  :description "show command"
  (interactive "P")
  (let* ((comm (oref transient-current-prefix scope))
         (comm-data
          (plist-get comm 'command))
         (command-str (if (string-match-p "$i" comm-data)
                          (format (replace-regexp-in-string
                                   "$i"
                                   (cond
                                    ((equal the-prefix-arg '(64)) "*")
                                    (t
                                     (flet ((quote-str (arg)
                                                       (pcase arg
                                       ...
                                       (quote-str (or (plist-get comm 'input)
                                                      (expand-file-name (read-file-name "File: ")))))))
                                   comm-data)
                                  (string-join (transient-args (oref transient-current-prefix command)) " "))
                        comm-data)))
    (funcall (cond
              ((member the-prefix-arg '((16) (64))) 'insert)
              ((equal the-prefix-arg '(4)) 'kill-new)
              (t 'detached-shell-command))
             command-str)))
#+end_src
...
#+begin_src emacs-lisp
(cl-defun mpv-command (&key file no-resume start end)
  ""
  (interactive)
  (transient-define-prefix my-shell-command/mpv ()
    ...
    [("p" "play" my/shell-command-string)
     ("d" "insert command for dired-*"
      (lambda () (interactive) (funcall-interactively 'my/shell-command-string '(64)))
      :if (lambda () (eq minibuffer-history-variable 'dired-shell-command-history)))])

  (transient-setup 'my-shell-command/mpv nil nil
                   :scope (let ((command-data (plist-put nil 'command "mpv %s $i")))
                            (if file
                                (plist-put command-data
                                           'input
                                           file)
                              command-data))))
...
(with-eval-after-load 'embark
  (mapc (lambda (map)
          (lambda-key map "P"
                      (lambda (file)
                        "play media FILE"
                        (mpv-command :file file :no-resume t))))
        (list embark-file-map embark-url-map))
  (with-eval-after-load 'org
    (mapc (lambda (map)
            (lambda-key map "P"
...
#+begin_src emacs-lisp
(cl-defun generate-audio-command (&key file bitrate tempo)
  ""
  (interactive)
  (transient-define-prefix my-shell-command/mpv ()
    "Prefix that just shows off many typical infix types."
    ...
    [("e" "execute" my/shell-command-string)
     ("d" "insert command for dired-*"
      (lambda () (interactive) (funcall-interactively 'my/shell-command-string '(64)))
      :if (lambda () (eq minibuffer-history-variable 'dired-shell-command-history)))])

  (transient-setup 'my-shell-command/mpv nil nil
                   :scope (let ((command-data (plist-put nil 'command
