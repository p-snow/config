...
:PROPERTIES:
:header-args+: :tangle (expand-file-name "my-general-config.el" my/user-config-dir)
:END:
...
**** Custom Variables
:PROPERTIES:
:header-args+: :tangle (expand-file-name "my-custom-variables.el" my/user-config-dir)
...
:PROPERTIES:
:header-args+: :tangle (expand-file-name "my-features.el" my/user-config-dir)
:END:
...
#+begin_src emacs-lisp :tangle (expand-file-name "org-web-track.el" my/user-package-dir)
  ...
  (defvar org-web-track-last-value-property "TRACK_VALUE_LAST" "docstring")
  ...
  (defun org-web-track-update ()
    "docstring"
    (interactive)
    (when-let* ((url (org-entry-get (point) org-web-track-url-property))
                (val (funcall (assoc-default url my/org-price-checker-retrievers #'string-match)
                              url)))
      (setq-local last-val (org-entry-get (point) org-web-track-last-value-property))
      (when (or (not last-val)
        ...
        (setq-local org-log-note-headings
                    (append (default-value 'org-log-note-headings)
                            `((price . ,(concat (format "Price %-8s" val)
                                                (if last-val
                                                    (format "from %-8s" last-val)
                                                  (make-string 13 ? ))
                                                " on %t")))))
        (org-add-log-setup 'price nil nil 'time)
...
#+begin_src shell :eval no :results verbatim :noweb yes :tangle (expand-file-name "web-track-mail" my/user-bin-dir) :tangle-mode (identity #o755)
...
#+begin_src emacs-lisp :tangle (expand-file-name "org-web-track-mail.el" my/user-batch-dir)
  (add-to-list 'load-path
               "~/.emacs.d/elpa/ov-20200326.1042/")
  (add-to-list 'load-path
               "~/.emacs.d/elpa/org-super-agenda-20220826.2315/")
  (add-to-list 'load-path
               "~/.emacs.d/elpa/peg-1.0.1")
  (add-to-list 'load-path
               "~/.emacs.d/elpa/org-ql-20221210.233/")
