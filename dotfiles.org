...
- Back-up Cinnamon Desktop Configuration
  #+begin_src shell :eval no :tangle no :noweb-ref dawn-batch
...
- Automating Script
  Update packages in base profile at dawn
  #+begin_src shell :eval no :tangle no :noweb-ref dawn-batch
...
elfeed-update will run at dawn everyday
...
(defun my/command-system-sleep (wake-at delay)
...
(dawn-job "dawn.sh;")
(weekend-job "which system.sh | xargs pwsudo; dawn.sh; weekend.sh;")
#+end_src
...
- when to dispatch dawn batch
  #+name: dawn-hour
  #+begin_src text
  4
  #+end_src

- dawn.sh: dawn batch script
  #+NAME: dawn-batch
  #+begin_src bash :noweb tangle :tangle (expand-file-name "dawn.sh" tangle/local-bin-dir)
  <<dawn-batch>>
  #+end_src
