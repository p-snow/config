...
#+end_src
**** dedicated major modes
:PROPERTIES:
:header-args+: :tangle (expand-tangle-target ".emacs.d/lisp/major-modes.el")
:END:
...
#+begin_src emacs-lisp
  (use-package dired
    :bind (:map dired-mode-map
                ("C-j" . my/dired-find-file-alternate-directory)
                ("^" . (lambda () (interactive)
                         (my/dired-find-file-alternate-directory "..")))
  ...
  (defun my/dired-find-file-alternate-directory (&optional file)
    "Visit file at current line with `find-file' or visit directory with `find-alternate-file'.

  If `FILE' is passed, find it instead."
    (interactive)
    (if-let ((find-file (or (and file
                                 (if (file-name-absolute-p file)
                                     file
                                   (expand-file-name file (dired-current-directory))))
                            (dired-get-filename nil t))))
        (cond
         ((file-directory-p find-file)
          (find-alternate-file find-file))
         ((file-readable-p find-file)
          (find-file find-file))
         (t
          (message "File is not readable.")))
      (message "Nothing to open in this line.")))
...
#+begin_src emacs-lisp
  (use-package embark
    ...
    :config
    (add-to-list 'embark-allow-edit-actions
...
**** managing, building and maintaining programs
:PROPERTIES:
:header-args+: :tangle (expand-tangle-target ".emacs.d/lisp/programs.el")
:END:
***** imenu
            ...
            (ytdl-opts
             (remove nil
                     (list (when (numberp pos)
                           ...
                           (format "--ytdl-format=%s" (shell-quote-argument (ytdl-format)))))))
...
***** say out loud text with speech synthesis

#+begin_src emacs-lisp
  (defcustom my/say-maximum-duration 30
    "Maximum speech duration in second for `my/say'.")

  (defun my/say (&optional arg)
    "Say out loud text in buffer.

  If region is active the selected string is spoken instead.

  With `C-u' prefix ARG, cancel speeching.

  As of now, English and Japanese is supported for speech. Appropriate language is
   determined from a spoken string, specifically the string consists of ASCII
   character is spoken in English. Otherwise the string is  spoken in Japanese."
    (interactive "P")
    (if (equal arg '(4))
        (progn
          (and my/say--timer
  ...
  (defun my/say-ascii-p (text)
    "Return non-nil if TEXT consists of ASCII character."
    (string-match-p
     (format "\\`[%s]+\\'" "[:ascii:]’“”–")
     text))
#+end_src
...
#+begin_src emacs-lisp
  (defun my/org-alter-heading (new-heading)
    "Replace heading of the org entry on which cursor points with NEW-HEADING."
    (interactive `(,(if (and (boundp 'new-heading)
                             (stringp new-heading))
                        new-heading
...
#+begin_src emacs-lisp :tangle (expand-tangle-target ".emacs.d/site-lisp/lfile.el")
  (defcustom my/lfile-root "~/doc" "docstring")

  (defun my/lfile-follow-link (file _)
    "Determine path from given basename FILE using `locate' program."
    (let ((paths (my/lfile--locate file)))
      (cond
       ((= 0 (length paths))
        (message "Sorry, no results found for query: %s" file))
       ((<= 1 (length paths))
        (my/view-file (car paths) current-prefix-arg)))))
  ...
  (defun my/lfile--locate (pattern)
    (split-string
     (shell-command-to-string
      (concat (format
               "locate -q -r ^%s.*"
               (shell-quote-argument (expand-file-name my/lfile-root)))
              (and pattern
...
It's more reasonable that Narrowing locate searching target to home directory.
...
- definitions
  ...
  #+name: locate-db
  #+begin_src emacs-lisp :noweb yes
    (expand-file-name "~/.local/share/mlocate/mlocate.db")
  #+end_src
...
- updatedb script for user home directory
  ...
  #+begin_src shell :noweb tangle :tangle (expand-tangle-target ".local/bin/updatedb_home") :shebang #!/usr/bin/env bash :tangle-mode (identity #o755)
    [[ -d $(dirname <<locate-db()>>) ]] || mkdir -p $(dirnmae <<locate-db()>>) && which updatedb | xargs -I {} pwsudo {} -U ~/ -l 0 -o <<locate-db()>>
  #+end_src
- updatedb cron job
...
#+begin_src conf :noweb yes :noweb-ref cronjob-pc
  20 * * * * /bin/bash -ci "pwsudo btrbk --config ~/.config/btrbk/btrbk.conf --loglevel error snapshot *-local >> $LOG_DIR/btrbk.log 2>&1"
#+end_src
...
#+begin_src conf :noweb tangle :tangle (expand-tangle-target ".config/btrbk/btrbk.conf")
  ...
  volume /mnt/ssd01
    subvolume incumbents/doc
      snapshot_name doc-local
      snapshot_preserve 7d 12w 12m
      snapshot_preserve_min latest
    subvolume incumbents/doc
      snapshot_name doc-backup
      target send-receive /mnt/hdd01/backups/ssd01
      target_preserve 12w *m
      target_preserve_min no
    subvolume incumbents/share
      snapshot_name share-local
      snapshot_preserve 60d 30w
      snapshot_preserve_min 1w
    subvolume incumbents/share
      snapshot_name share-backup
      target send-receive /mnt/hdd01/backups/ssd01
      target_preserve *w
      target_preserve_min no

  volume /mnt/nvme01
    subvolume incumbents/work
      snapshot_name work-local
      snapshot_preserve 5d 4w
      snapshot_preserve_min 1d
    subvolume incumbents/work
      snapshot_name work-backup
      target send-receive /mnt/hdd01/backups/pc01
      target_preserve 10w
      target_preserve_min latest
    subvolume incumbents/media
      snapshot_name media-local
      snapshot_preserve 12h 3d 2w 1m
      snapshot_preserve_min latest
#+end_src
...
#+begin_src shell :tangle (expand-tangle-target ".local/bin/dvdpack") :tangle-mode (identity #o555)
    ...
    hb_opts='--x264-preset veryslow --quality 18 --aencoder copy --main-feature --markers --all-audio --all-subtitles'
...
#+begin_src shell :tangle (expand-tangle-target ".local/bin/av1encode") :tangle-mode (identity #o555)
  ...
  av1_opts_common="-c:v libaom-av1 -cpu-used 4 -row-mt 1 -threads $(nproc) -b:v 0 -crf ${CRF}"
