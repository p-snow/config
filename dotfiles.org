...
#+begin_src emacs-lisp
  (defun my/command-system-sleep (&optional sleep-delay wake-at)
    ""
    (interactive (list (read-number "Delay before sleep in sec: ")
                       (read-number "Wake at: ")))
    (format "sleep %s && pwsudo rtcwake -m mem -s %s"
            sleep-delay wake-at))
#+end_src
