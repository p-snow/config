...
***** show sparse tree for the entry at point
:PROPERTIES:
:CREATED:  [2023-11-01 Wed 14:48]
:END:

#+begin_src emacs-lisp
  (defun my/org-sparse-tree-at-point ()
    "Show sparse tree for the entry at point."
    (interactive)
    (save-excursion
...
#+begin_src conf :noweb yes :noweb-ref cronjob-user
  2 4 * * 0-5 /bin/bash -ci midnight.sh
#+end_src
...
#+begin_src conf :noweb yes :noweb-ref cronjob-user
  2 4 * * 6 /bin/bash -ci "midnight.sh; weekend.sh"
#+end_src
...
#+end_src
** Low Power Computing During The Night
:PROPERTIES:
...
#+begin_src bash :eval no :noweb tangle :tangle (expand-file-name ".local/bin/syssleep-after.sh")
  date_str=$(
    if [[ $(date +%H) -lt 4 ]]; then
      date -d "today 4am" +"%Y-%m-%d %H:%M"
    else
      date -d "tomorrow 4am" +"%Y-%m-%d %H:%M"
    fi
  )
