...
(use-package gptel
  ...
  (cl-pushnew '(proofreading
              ...
              gptel-directives)
  (cl-pushnew '(document
                . "You are a large language model and a software engeneer. Please rewrite the following snippet of the software document.
- If the text starts with a verb, it may the description for a function.")
              gptel-directives)
...
- Interpret the sentence in Japanese if it is English.
- Interpret the sentence in English if it is Japanese.")
              gptel-directives)
  ...
  (and-let* ((value (oref obj value)))
    (let ((arg (oref obj argument)))
      (pcase-exhaustive (oref obj multi-value)
        ...
        ((or 't 'rest) (concat arg (mapconcat 'shell-quote-argument value " ")))
        ;; FIXME: this may fail
        ('repeat       (mapcar (lambda (v) (cons arg (shell-quote-argument v))) value))))))
