...
(with-eval-after-load 'embark
  (mapc (lambda (map)
          (lambda-key map "D"
                      (lambda (url)
                        "download media file"
                        (funcall-interactively 'my/command-download-media url)))
          (lambda-key map "P"
                      (lambda (url)
                        "download and play media file"
                        (funcall-interactively 'my/command-download-media url :play t))))
        (list embark-kill-ring-map embark-url-map)))

(with-eval-after-load 'my-launch-app
  (keymap-set 'my/launch-app-map "D"
...
(transient-define-suffix my/command-execute ()
  ...
  (let* ((cur-obj (transient-suffix-object))
    ...
    (funcall (oref cur-obj dispatch-command)
             (format-spec
