    ...
    (let* ((match-idx (string-match "^\\([^:]*\\)\\(::?\\(.*\\)\\)?$" tag))
      ...
      (concat (string-trim
               (shell-command-to-string
                (concat (format "plocate -d <<plocate-db()>> -ie \"%s\" 2>/dev/null"
  ...
  (setq org-web-track-trackers
        `((,(rx "www.amazon.co.jp"
          ...
          (,(regexp-quote "store.steampowered.com")
           ("price" . "pup 'div[class=\"discount_final_price\"] json{}' | jq -r .[0].text"))))
#+end_src
...
#+begin_src emacs-lisp :noweb tangle :tangle (expand-file-name "my-org-config.el" my/user-config-dir)
  (use-package org-web-track
    ...
    (defun my/org-web-track-list ()
      ...
      (let ((org-agenda-files
        ...
        (when (require 'org-ql nil t)
          (org-ql-search
            (org-agenda-files)
            `(property ,org-web-track--propname-value)
    ...
    (bind-keys ("C-o W W" . my/org-web-track-list)
               ("C-o W n" . org-web-track-initialize)
...
https://www.amazon.co.jp/%E3%81%AF%E3%81%8F%E3%81%B0%E3%81%8F-%E4%B8%B8%E7%B2%92%E9%BA%A6%E8%8C%B6-30g%C3%9730%E8%A2%8B/dp/B0087WQ6NI/ref=asc_df_B0087WQ6NI/?tag=jpgo-22&linkCode=df0&hvadid=204320102829&hvpos=&hvnetw=g&hvrand=6273707311668630873&hvpone=&hvptwo=&hvqmt=&hvdev=c&hvdvcmdl=&hvlocint=&hvlocphy=1009441&hvtargid=pla-338981819961&psc=1&th=1&psc=1
