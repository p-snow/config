...
*** basic configuration
:PROPERTIES:
:header-args+: :tangle (expand-file-name "my-config.el" tangle/emacs-config-dir)
:CREATED:  [2023-01-20 Fri 10:22]
:END:

**** early-init.el
:PROPERTIES:
:header-args+: :tangle (expand-file-name "early-init.el" user-emacs-directory) :tangle-mode (identity #o644)
:END:
...
(setopt package-user-dir
        (no-littering-expand-var-file-name "elpa"))
(setopt package-gnupghome-dir
        (no-littering-expand-var-file-name "elpa/gnupg"))
...
(let ((load-match "\\.el$"))
  (mapc #'load-file
        (append (directory-files (file-name-concat user-emacs-directory "lisp") t load-match)
                (directory-files (file-name-concat user-emacs-directory "lisp/tangled") t load-match))))
#+end_src
...
- bind find-functions and finder commands
  ...
  (define-key ctl-x-map "C" 'finder-commentary)
  #+end_src
...
(use-package recentf
  ...
  (recentf-auto-cleanup (* 7 60)) ; clean-up when idle time reaches 7 minutes
  :config
  (recentf-mode 1))
#+end_src
...
If GC time at Emacs init is problematic, checking [[help:gcs-done][gcs-done]] and [[help:gc-elapsed][gc-elapsed]] may help you. Increasing gc-cons-* variables would minimize Emacs init time, but this also cause each GC time which may noticeable as freeze.
[[https://emacsconf.org/2023/talks/gc/][
This presentation]] advise to increase gc-cons-threshold to 20-80Mb if you have Emacs with slow startup, and to increase consult--gc-percentage to 0.2-1.0 if you have slow and frequent GC.
...
(use-package midnight
  ...
  :custom
  (midnight-mode t)
  (midnight-delay "<<morning-hour>>:00"))
#+end_src
...
- ddskk init file
#+begin_src emacs-lisp :tangle (no-littering-expand-etc-file-name "ddskk/ddskk-init.el")
;; -*- mode:emacs-lisp; -*-
...
#+NAME: eww-org-dir
#+begin_src emacs-lisp
(no-littering-expand-var-file-name "eww/org")
#+end_src

#+begin_src conf :noweb yes :noweb-ref tmpfiles-cleanup :tangle no
d <<eww-org-dir()>> 0755 - - 3d
#+end_src
...
(defun elfeed-search-show-media-duration ()
  ...
  (let* ((entry (elfeed-search-selected :single))
         (url (or (caar (elfeed-entry-enclosures entry))
                  (elfeed-entry-link entry))))
    (let ((my/execute-command-function '(shell-command-to-string string-trim princ)))
...
export DISPLAY=:0 && \
  emacs -Q --batch \
        ...
        -l $XDG_CONFIG_HOME/emacs/lisp/batch/org-web-track-mail.el \
  ...
  (defun my/define-word ()
    ...
    (let ((word (if (use-region-p)
                    (buffer-substring-no-properties
                     (region-beginning)
                     (region-end))
                  (read-string "Word: " nil 'my/define-word-minibuffer-history
                               (word-at-point t)))))
...
(use-package tempel
  ...
  :custom
  (tempel-path (list (no-littering-expand-var-file-name "tempel/templates/*.eld")
                     (no-littering-expand-var-file-name "tempel/templates/*/*.eld")))
  :config
...
(use-package logos
  ...
  :hook ((help-mode helpful-mode Info-mode eww-mode nov-mode mu4e-view-mode woman-mode mastodon-mode Custom-mode)
         . logos-focus-mode)
  :custom
...
(use-package yeetube
  ...
  :bind (:map engine-mode-prefixed-map
              ("Y" . yeetube-search))
  :custom
  (yeetube-download-directory (file-name-concat "<<media-dir()>>" "downloads"))
...
#+begin_src emacs-lisp :noweb tangle
;; (setopt gnuplot-program "<<which(filename="gnuplot")>>")
#+end_src
...
***** Variables for tangling
:PROPERTIES:
:CREATED:  [2024-02-05 Mon 20:43]
:END:

#+begin_src emacs-lisp :tangle (expand-file-name "early-init.el" user-emacs-directory)
(setq tangle/home "~/dotfiles/")
(setq tangle/user-emacs-directory (file-name-concat tangle/home ".config/emacs"))
(setq tangle/emacs-sitelisp-dir
      (expand-file-name "site-lisp" tangle/user-emacs-directory))
(setq tangle/emacs-config-dir
      (expand-file-name "lisp" tangle/user-emacs-directory))
(setq tangle/emacs-etc-dir
      (expand-file-name "etc" tangle/user-emacs-directory))
(setq tangle/emacs-var-dir
      (expand-file-name "var" tangle/user-emacs-directory))
(setq tangle/local-bin-dir
      (expand-file-name ".local/bin/" tangle/home))
#+end_src
...
#+begin_src emacs-lisp :comments no :tangle (expand-file-name "org-babel-config.el" tangle/emacs-config-dir) :noweb-ref babel-conf
(with-eval-after-load 'ob-core
...
(defclass dispatch-shell-command (transient-suffix)
  ((shell-command-string :initarg :shell-command-string)
   (format-spec :initarg :format-spec :initform nil)
   (dispatch-command :initarg :dispatch-command)))

(cl-defmethod transient-init-value ((obj dispatch-shell-command))
  (oset obj dispatch-command (my/determine-invocation-function)))

(defalias 'dispatch-execute-macro (kmacro "x"))
...
(transient-define-suffix dispatch-dwim-execute ()
  ...
  (let* ((cur-obj (transient-suffix-object))
         (format-spec (and (slot-boundp cur-obj :format-spec)
                           (oref cur-obj format-spec)))
         (comm (format-spec
                (eval (oref cur-obj shell-command-string))
                (append
                 `((?a . ,(string-join (transient-get-value) " ")))
                 (when format-spec
                   (mapcar (lambda (key-val) (cons (car key-val) (eval (cdr key-val))))
                           format-spec))))))
    (if dispatch-dwim--stacking
        (progn
          (setq dispatch-dwim--stacking nil)
          (setq dispatch-dwim--stack-message comm))
      (pcase (oref cur-obj dispatch-command)
        ((and (pred symbolp) sym) (funcall sym comm))
        ((and (pred listp) li)
         (seq-reduce (lambda (accum fun) (funcall fun accum))
                     li comm))))))
...
(defvar my/execute-command-function
  'detached-shell-command)

(defun my/determine-invocation-function ()
  (if (eq major-mode 'minibuffer-mode)
      'insert
    my/execute-command-function))

(defun dispatch-command-dwim/determine-file (&optional file)
  ""
  (cl-labels ((read-file ()
                (read-file-name "File: ")))
    (apply (or (and file
                   ...
                   (with-current-buffer (other-buffer nil t)
                     `(dispatch-dwim--paths-string
                       ,(if (derived-mode-p 'dired-mode)
                            (or (dired-get-marked-files) (read-file))
                          (read-file)))))))
                (t `(dispatch-dwim--paths-string ,(read-file))))))))

(defun dispatch-dwim--determine-url (&optional url)
  "Determine the URL to which the dwim commands refer in relation to the context."
  (format "'%s'" (or url
                     (thing-at-point-url-at-point)
...
(defun dispatch-dwim--paths-string (arg)
  (pcase arg
    ((and (pred stringp) (pred file-exists-p) str)
     (shell-quote-argument (expand-file-name str)))
    ((and (pred stringp) (pred (string-match-p ffap-url-regexp)) (app (match-string 1) str))
     (dispatch-dwim--determine-url str))
    ((and (pred listp) li) (mapconcat #'dispatch-dwim--paths-string li " "))))

(defvar dispatch-dwim--stack-message nil)
(defvar dispatch-dwim--stacking nil)

(defclass dispatch-dwim-sub-suffix (transient-suffix)
  ((message   :initarg :message)
   (format    :initarg :format   :initform " %k %d (%v)")))

(cl-defmethod transient-infix-value ((obj dispatch-dwim-sub-suffix))
  ""
  (when (slot-boundp obj :message)
    (oref obj message)))

(cl-defmethod transient-format-value ((obj dispatch-dwim-sub-suffix))
  (if (slot-boundp obj :message)
      (propertize (oref obj message)
                  'face 'transient-argument)
    ""))

(cl-defmethod transient-format ((obj dispatch-dwim-sub-suffix))
  (when (and (not dispatch-dwim--stacking)
             (stringp dispatch-dwim--stack-message))
    (oset obj message dispatch-dwim--stack-message)
    (setq dispatch-dwim--stack-message nil))
  (format-spec (oref obj format)
               `((?k . ,(transient-format-key obj))
                 (?d . ,(transient-format-description obj))
                 (?v . ,(transient-format-value obj)))))

(transient-define-suffix dispatch-dwim-stack-command ()
  :class dispatch-dwim-sub-suffix
  :transient 'transient--do-recurse
  (interactive)
  (setq dispatch-dwim--stacking t)
  (setq dispatch-dwim--stack-message nil)
  (dispatch-dwim-select))
...
#+begin_src emacs-lisp
(transient-define-prefix mpv-dispatch-dwim (&optional file no-resume start end)
  ""
  :man-page "mpv"
  ...
  (transient-setup 'mpv-dispatch-dwim nil nil
                   :scope (plist-put nil 'file (dispatch-command-dwim/determine-file file))))
...
#+begin_src emacs-lisp
(transient-define-prefix ffmpeg-opus-dispatch-dwim (&optional file)
  ""
...
#+begin_src emacs-lisp
(transient-define-prefix ffmpeg-vp9-dispatch-dwim (&optional file)
  ""
  ...
  [(dispatch-dwim-execute
    :shell-command-string
    (format "ffmpeg -i %5$s %1$s %2$s /dev/null && ffmpeg %%S -i %5$s %1$s %3$s %4$s %5$s_out.webm"
            "-c:v libvpx-vp9 -cpu-used 4 -threads 3 -row-mt 1 -speed 1 -b:v 0 %c"
            "-pass 1 -an -f null"
            "-pass 2 -map_metadata 0"
            "-c:a libopus %b -ac 2"
            (plist-get (oref transient-current-prefix scope) 'file))
    :format-spec ((?b . (dispatch-shell-command--key-vals '("-B")))
...
(transient-define-prefix sleep-dispatch-dwim (&optional number)
  ""
  :man-page "sleep"
  [("-b" "time before sleep" "" :prompt "amount of time [s/m/h/d]: "
    :class transient-option :always-read t :allow-empty nil
    :init-value (lambda (obj) (oset obj value (or number "0"))))]
  [(dispatch-dwim-execute
    :shell-command-string "sleep %a")]
  (interactive)
  (transient-setup 'sleep-dispatch-dwim))

(transient-define-prefix rtcwake-complex-dispatch-dwim (&optional number)
  ""
  :man-page "rtcwake"
  [("-t" "wake-up time" "--date " :prompt "Wake-up time: " :class transient-option
    ...
    :init-value (lambda (obj) (oset obj value (my/transient-read-time nil "+7h" nil))))]
  [("p" "pre task" dispatch-dwim-stack-command
    :message "sleep 30m")
   ("P" "post task" "" :prompt "Wake-up job: " :class transient-option
    :choices ("morning.sh" "which system.sh | xargs pwsudo; morning.sh; weekend.sh")
    :init-value (lambda (obj) (oset obj value (car (oref obj choices)))))
   (dispatch-dwim-execute
    :shell-command-string "%p; pwsudo rtcwake -l -m mem -v %d; %P"
    :format-spec ((?d . (dispatch-shell-command--key-vals '("-t")))
                  (?P . (dispatch-shell-command--key-vals '("P")))
                  (?p . (dispatch-shell-command--key-vals '("p")))))]
  (interactive)
  (transient-setup 'rtcwake-complex-dispatch-dwim))
...
#+name: pandoc-command
#+begin_src emacs-lisp
(transient-define-prefix pandoc-dispatch-dwim (&optional file)
  ""
  [(my/pandoc-to)]
  [(dispatch-dwim-execute
    :shell-command-string
    (format "parallel pandoc %%a '{}' ::: %s"
            (plist-get (oref transient-current-prefix scope) 'file)))]
  (interactive)
  (transient-setup 'pandoc-dispatch-dwim nil nil
                   :scope (plist-put nil 'file (dispatch-command-dwim/determine-file file))))

(defclass my/transient-option (transient-option)
  ())

(transient-define-infix my/pandoc-to ()
  :class my/transient-option
  :transient t
  :prompt "To format: "
  :key "-t"
  ...
  :always-read t
  :init-value (lambda (obj) (oset obj value (car (oref obj choices))))
  :choices '("org" "pdf" "epub" "rst" "texinfo" "odt" "docx"))

(cl-defmethod transient-infix-value ((obj my/transient-option))
  ""
  (let ((fmt (oref obj value)))
    (concat (format "-o '{.}'.%s" fmt)
            (when (string= fmt "pdf")
...
#+name: yt-dlp-command
#+begin_src emacs-lisp :noweb tangle
(transient-define-prefix yt-dlp-dispatch-dwim (&optional url output duration-p play-p)
  ""
  :man-page "yt-dlp"
  [("-P" "download path" "--paths "
    :class transient-option
    :reader transient-read-existing-directory
    :init-value (lambda (obj) (oset obj value (file-name-concat "<<media-dir()>>" "downloads"))))
   ...
   ("-S" "format sort" "--format-sort "
    :class transient-option
    :init-value (lambda (obj) (oset obj value "'filesize:2G'")))
   ("-o" "output" "--output "
  ...
  [(dispatch-dwim-execute
    ;; embed url to :shell-command-string may cause unexpected glitch
    :shell-command-string
    "yt-dlp --embed-chapters --live-from-start %a %u"
    :format-spec ((?u . (plist-get (oref transient-current-prefix scope) 'url))))]
  (interactive)
  (setq my/detached-bury-buffer t)
  (transient-setup 'yt-dlp-dispatch-dwim nil nil
                   :scope (plist-put nil 'url
...
#+begin_src emacs-lisp
(transient-define-prefix du-dispatch-dwim (&optional file)
  "Dispatch du command."
  :man-page "du"
...
#+begin_src emacs-lisp :noweb yes :tangle (no-littering-expand-var-file-name "tempel/templates/fundamental.eld")
...
#+begin_src emacs-lisp :noweb yes :tangle (no-littering-expand-var-file-name "tempel/templates/text.eld")
...
#+begin_src emacs-lisp :noweb yes :tangle (no-littering-expand-var-file-name "tempel/templates/org.eld")
...
#+begin_src emacs-lisp :noweb yes :tangle (no-littering-expand-var-file-name "tempel/templates/minibuffer.eld")
...
#+begin_src emacs-lisp :noweb yes :tangle (no-littering-expand-var-file-name "tempel/templates/emacs-lisp.eld")
...
#+begin_src emacs-lisp :noweb yes :tangle (no-littering-expand-var-file-name "tempel/templates/sh-base.eld")
(rf "${" p "}")
...
#+begin_src emacs-lisp :noweb yes :tangle (no-littering-expand-var-file-name "tempel/templates/mu4e-compose.eld")
(よろ "よろしくお願いいたします。")
...
(weekend-job "which system.sh | xargs pwsudo; morning.sh; weekend.sh;")
#+end_src
...
#+begin_src conf :noweb yes :noweb-ref tmpfiles-cleanup :tangle no
d <<emacs-var-dir()>>/org/subtree-view 0755 - - 3d
#+end_src
...
(defvar my/org-subtree-view-directory
  (no-littering-expand-var-file-name "org/subtree-view")
...
**** read well-used org files insidiously
:PROPERTIES:
:CREATED:  [2023-11-28 Tue 07:38]
:END:

#+begin_src emacs-lisp
(use-package read-org-agenda-files
  :no-require t
  :after org
  :config
  (run-with-idle-timer (* 30) nil
                       (lambda ()
                         "Read org agenda files."
...
(no-littering-expand-var-file-name "open-file")
#+end_src
...
- make some projects invisible from plocate command
  #+NAME: ghq-invisible-projects
  #+begin_src shell :tangle no
  ghq list --full-path p-snow/dotfiles
  #+end_src

  #+begin_src shell :noweb yes :noweb-ref updatedb-opt
  -e "<<ghq-invisible-projects()>>" \
  #+end_src
...
#+NAME: bash/emacs-yt-dlp
#+begin_src bash :noweb tangle :tangle (expand-file-name "emacs-yt-dlp" tangle/local-bin-dir)
if [ $# -ge 1 ]; then
  arg=" nil nil t"
else
  arg=""
