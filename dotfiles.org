...
#+begin_src conf :noweb yes :noweb-ref cronjob-user
  ,*/15 * * * * <<which(filename="bash")>> -ci "btrbk_home >> $LOG_DIR/btrbk_home.log 2>&1"
#+end_src
...
#+begin_src emacs-lisp :noweb tangle
  (use-package midnight
    ...
    :custom
    (midnight-mode t)
    (midnight-delay "<<midnight-hour>>:10"))
#+end_src
      ...
      (cl-loop for entry in entries
               ...
               do (let ((title (elfeed-entry-title entry)))
                    (detached-shell-command
                     (my/command-download-media it title))))
...
#+begin_src emacs-lisp
  ...
  (use-package org-agenda
    ...
    (org-agenda-text-search-extra-files
     (mapcan (lambda (dir)
               (directory-files (org-file-name-concat org-directory
                                                      dir)
                                t
                                ".+\\(?:\\.org\\(?:\\.gpg\\)?\\)$"
                                ))
            ...
            ("i" "Today's agenda"
             ((todo "Today's agenda"
              ...
              (org-ql-search-block `(and (planning :on today)
                                         ...
                                         (not (tags "web"))
                                         (not (habit))
                                         (not (and (todo ,my/org-todo-keyword--someday)
                                                   (tags ,my/org-review-tag))))
...
#+begin_src emacs-lisp
  (use-package tempel
    ...
    :custom
    (tempel-path (file-name-concat user-emacs-directory-default "templates/*.eld"))
    :config
...
#+begin_src emacs-lisp
  ...
  (defun my/file-name-legitimate (filename)
    "Replace / with ⧸ in `FILENAME' so that it can be used as an file name in Linux file system."
      ...
      (cl-labels ((lookup-parent ()
                    (let* ((heading (substring-no-properties
                      ...
                      (if (org-up-heading-safe)
                        ...
                        h-li))))
        (org-ql-sparse-tree (message "(olps %s)"
...
#+begin_src emacs-lisp :noweb yes :tangle (file-name-concat user-emacs-directory-default "templates/text.eld")
  (cm "update on " (my/insert-time-stamp '(4))
      " at " (system-name)
      " by " (user-full-name))
#+end_src
...
#+begin_src emacs-lisp
  (defun my/command-system-sleep (&optional delay wake-at)
    "Return a shell command string to sleep system until WAKE-AT with DELAY time."
    (interactive (list (read-string "Delay before sleep: ")
                       (read-string "Wake time: ")))
    (string-trim-right
     (format "sleep %s && pwsudo rtcwake -m mem -l -t %s"
             delay
             (shell-command-to-string (format "date +%%s -d '%s'" wake-at)))))
#+end_src

#+begin_src emacs-lisp :tangle no :noweb-ref minibuffer-template
  (slp (call-interactively 'my/command-system-sleep))
#+end_src
...
command builders
#+begin_src emacs-lisp :noweb tangle
  (defun my/command-download-media (&optional url filename)
    "Return a shell command string to download media whose name is FILENAME from URL using yt-dlp."
    (interactive (list (read-string "Download URL: ")))
    (format "yt-dlp %s"
            (string-join
             (delq nil
                   (list (format "-P '%s'" (file-name-concat "<<media-dir()>>" "downloads"))
                         (and filename
                              (format "-o '%s.%%(ext)s'" (my/file-name-legitimate filename)))
                         (format "'%s'" url)))
             " ")))
  ...
  (defun my/command-media-duration (&optional url)
    "Return a shell command string to get duration of media file at URL."
    (interactive (list (read-string "Download URL: ")))
    (format "yt-dlp %s"
            (string-join
             (delq nil
...
#+begin_src emacs-lisp :tangle no :noweb-ref minibuffer-template
  (mdl (call-interactively 'my/command-download-media))
  (m= (call-interactively 'my/command-media-duration))
#+end_src
...
- cron job for midnight.sh
  #+begin_src conf :noweb yes :noweb-ref cronjob-user
    2 <<midnight-hour>> * * 0-5 <<which(filename="bash")>> -ci "midnight.sh"
  #+end_src
...
#+begin_src conf :noweb yes :noweb-ref cronjob-user
  2 <<midnight-hour>> * * <<weekend-day>> <<which(filename="bash")>> -ci "midnight.sh; weekend.sh"
#+end_src
...
#+end_src
** System Sleep During The Night
:PROPERTIES:
:CREATED:  [2023-11-01 Wed 16:45]
:END:
...
#+begin_src bash :eval no :noweb tangle :tangle (expand-file-name ".local/bin/syssleep-after.sh")
  date_str=$(
    if [[ $(date +%H) -lt 4 ]]; then
      date -d "today <<midnight-hour>>" +"%Y-%m-%d %H:%M"
    else
      date -d "tomorrow <<midnight-hour>>" +"%Y-%m-%d %H:%M"
    fi
  )

  if [ -v 1 ]; then
    sleep $1 && pwsudo rtcwake -m mem --date ${date_str}
  else
    pwsudo rtcwake -m mem --date ${date_str}
  fi
#+end_src
