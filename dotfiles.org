...
*** basic configuration
:PROPERTIES:
:header-args:emacs-lisp+: :tangle (expand-file-name "my-config.el" tangle/emacs-config-dir)
:CREATED:  [2023-01-20 Fri 10:22]
:END:
...
:header-args:emacs-lisp+: :tangle (expand-file-name "my-org-config.el" tangle/emacs-config-dir)
:END:
...
(defun my/tts-say-it (text)
  ...
  (interactive (list (cond
                      ((region-active-p)
                       (buffer-substring-no-properties (region-beginning) (region-end)))
                      (sentence-at-point))))
  (tts-openai-request (my/credential-info)
                      text
                      #'my/tts-play))
