...
- emacs
  ...
  #+end_src
- firefox
  a guix profile dedicated for firefox since building firefox takes tremendous time and requires 15GB+ memory.
  #+begin_src scheme :noweb yes :noweb-prefix no :tangle (expand-file-name ".config/guix/manifests/firefox.scm")
    (specifications->manifest
     '("firefox"))
  #+end_src
*** Activating Profiles
...
#+begin_src emacs-lisp
  ...
  (defvar my/org-remove-old-state-changes-weeks 30
...
#+begin_src emacs-lisp :tangle (expand-file-name "my-org-config.el" my/user-config-dir)
  (use-package org-drill-english
    :commands my/org-english-insert-japanese-pronunciation
...
- git config
  ...
  #+BEGIN_SRC gitconfig :noweb tangle :tangle (expand-file-name ".config/git/config")
    ...
    [commit]
            gpgsign = true
    [credential]
            helper = <<which(pass-git-helper)>> -m <<home-dir()>>/.config/pass-git-helper/git-pass-mapping.ini
    <<git-config>>
  #+END_SRC
- git attributes
...
#+begin_src conf :tangle (expand-file-name ".recoll/recoll.conf")
  topdirs = ~/doc-pc01/doc ~/data

  [~/doc-pc01/doc/]
  followLinks = 1

  skippedNames = .git open-file org-subtree-view
