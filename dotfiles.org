...
#+begin_src emacs-lisp
(transient-define-prefix dispatch/sleep-rtcwake (&optional number)
  ""
  :man-page "rtcwake"
  ...
  (interactive)
  (transient-setup 'dispatch/sleep-rtcwake))
                  ...
                  (let ((time-str (string-trim
                    ...
                    (if (or (and (string-prefix-p "date: invalid date"
                            ...
                            (and (time-less-p (time-add nil (seconds-to-time (* 12 60 60)))
                                 ...
                                 (message "Wake up time must be less than 12h later")))
...
(with-eval-after-load 'embark
  (mapc (lambda (map)
          ...
          (lambda-key map "P"
                      (lambda (url)
                        "download and play media file"
                        (dispatch-yt-dlp-dwim url nil nil t)
                        (dispatch-execute-macro))))
        (list embark-kill-ring-map embark-url-map)))

(with-eval-after-load 'my-launch-app
  (keymap-set 'my/launch-app-map "D D"
              (defalias 'download-media-from-kill-ring
                (kmacro "C-M-y h t t p C-. D")))
  (keymap-set 'my/launch-app-map "D P"
              (defalias 'play-media-from-kill-ring
                (kmacro "C-M-y h t t p C-. P"))))
#+end_src
...
#+begin_src emacs-lisp
(command-defun my/region-string-trans-buffer ()
