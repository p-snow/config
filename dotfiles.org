...
#+begin_src shell :noweb tangle :tangle (expand-file-name ".local/bin/initialize-ubuntu-desktop") :shebang #!/bin/sh :tangle-mode (identity #o755)
...
[[https://github.com/victorhge/iedit][IEdit]] is not only replacement for M-%/C-M-% but it highlights symbol occurences to change. [[https://www.masteringemacs.org/article/iedit-interactive-multi-occurrence-editing-in-your-buffer][An adviced]] custom function 'my/iedit-dwim' replaces symbols in defun unless prefix arg is given.
...
#+begin_src emacs-lisp
  ...
  ;; require clause sits here to supress key occupation warning
  (require 'iedit)
...
#+begin_src emacs-lisp :tangle (expand-file-name ".emacs.d/site-lisp/org-english.el")
  ...
  (defun my/org-english-level (word)
    ...
    (let ((str (my/org-english-insert-japanese-pronunciation word)))
      (string-to-number
       (if (string-match
            (rx (seq "【レベル】"
  ...
  ;; TODO: This fun is NOT practical because of network cost.
...
- download hts voices
  ...
  #+begin_src shell :noweb tangle :tangle (expand-file-name ".local/bin/download-hts-voices") #!/usr/bin/env bash :tangle-mode (identity #o755)
