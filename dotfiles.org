...
#+begin_src emacs-lisp :tangle (expand-file-name "org-web-track.el" my/user-package-dir)
  (defvar org-web-track-url-property "TRACK_URL" "docstring")
  (defvar org-web-track-last-value-property "TRACK_VALUE" "docstring")

  (defvar my/org-price-checker-retrievers
    '(("www\.amazon\.co\.jp" . my/org-price-checker-retriever-amazon-jp))
    "An alist.")
  ...
  (defun org-web-track-update ()
    ...
    (let ((url (org-entry-get (point) org-web-track-url-property))
          ...
          (last-val (org-entry-get (point) org-web-track-last-value-property)))
      (unless (or (and (not url)
        ...
        (and (not (setq-local val (funcall (assoc-default url my/org-price-checker-retrievers #'string-match)
                                           url)))
        ...
        (when (or (not last-val)
          ...
          (org-entry-put (point) org-web-track-last-value-property val)
  ...
  (defun my/org-price-checker-retriever-amazon-jp (url)
    "docstring"
    (concat (seq-mapcat (lambda (char)
                          (string-replace "\\" "¥"
  ...
  (let (mail-msg)
    (mapc (lambda (price-entry)
            (let* ((heading (org-element-property :raw-value price-entry))
                   ...
                   (value (funcall (assoc-default track-url my/org-price-checker-retrievers #'string-match)
                                   track-url)))
          ...
          (org-ql-select
            (directory-files-recursively org-directory
                                         (rx (seq ".org" eol)))
            `(and (property ,org-web-track-url-property)
