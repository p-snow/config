...
#+begin_src conf :noweb tangle :tangle (expand-file-name "etc/keyd/default.conf" tangle/system)
[ids]
...
' = overloadt2(apostrophe_layer, ', 200)
...
(use-package tab-bar
  ...
  (defun my/tab-bar-mode-line-string ()
    "Return the first 3 characters of the name for the current tab."
    (substring
     (assoc-default 'name
          ...
          ("d" "Done tasks"
           ((org-ql-search-block `(and (or (done)
                                           (and (todo ,my/org-todo-keyword--someday)
                                                (not (path ,(expand-file-name "agenda/somedays.org" org-directory)))))
                                       (not (tags "project")))
...
***** Increasing a number at point preserving pad
:PROPERTIES:
...
#+begin_src emacs-lisp
(defun my/increase-number-at-point-preserve-pad (&optional inc)
  "Increment the number at point while preserving zero padding.
With an optional prefix numeric argument INC, increment using
this numeric value.

This command is a homage to org-increase-number-at-point."
  (interactive "p")
  (if (not (number-at-point))
    ...
    (let* ((pos (point))
      ...
      (insert (format (concat "%0" (format "%d" digit-count) "d")
                      (+ (string-to-number nap) inc))))))

(defun my/decrease-number-at-point-preserve-pad (&optional inc)
  "Decrement the number at point while preserving zero padding.
With an optional prefix numeric argument INC, decrement using
this numeric value."
  (interactive "p")
  (my/increase-number-at-point-preserve-pad (- (or inc 1))))

(keymap-global-set "C-x <up>" #'my/increase-number-at-point-preserve-pad)
(keymap-global-set "C-x <down>" #'my/decrease-number-at-point-preserve-pad)
(defvar-keymap my/increase-number-repeat-map
  :doc "Keymap to repeat increasing a number."
  :repeat (:enter (my/increase-number-at-point-preserve-pad
                   my/decrease-number-at-point-preserve-pad))
  "<up>" #'my/increase-number-at-point-preserve-pad
  "<down>" #'my/decrease-number-at-point-preserve-pad)
#+end_src
...
(transient-define-prefix rtcwake-dispatch-dwim (&optional number)
  ...
  [("-t" "wake-up time" "--date " :prompt "Wake-up time: " :class transient-option
    ...
    :init-value (lambda (obj) (oset obj value (my/transient-read-time nil "+6h15m" nil))))]
...
(transient-define-prefix yt-dlp-dispatch-dwim (url &optional output duration-p play-p)
  ...
  [("-P" "download path" "--paths "
   ...
   ("-F" "free format" "--prefer-free-formats"
    :init-value (lambda (obj) (oset obj value "--prefer-free-formats")))
   ...
   ("-S" "format sort" "--format-sort "
    :class transient-option
    :init-value (lambda (obj) (oset obj value "'codec,ext,size:1.5G'")))
   ("-o" "output" "--output "
...
(defvar-keymap my/org-move-repeat-map
  ...
  "M-F" #'org-babel-next-src-block
  "M-B" #'org-babel-previous-src-block)
#+end_src
