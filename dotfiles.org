...
#+begin_src shell :noweb tangle :tangle (expand-tangle-target ".local/bin/initialize-ubuntu-desktop") :shebang #!/bin/sh :tangle-mode (identity #o755)
...
#+begin_src shell :noweb tangle :tangle (expand-tangle-target ".local/bin/install-apt-packages") :shebang #!/bin/sh :tangle-mode (identity #o755)
...
#+begin_src shell :noweb tangle :tangle (expand-tangle-target ".local/bin/initialize-linuxmint-desktop") :shebang #!/bin/sh :tangle-mode (identity #o755)
...
#+begin_src shell :noweb tangle :tangle (expand-tangle-target ".local/bin/start-systemd-services") :shebang #!/bin/sh :tangle-mode (identity #o755)
  <<systemd-service>>
#+end_src
...
#+begin_src emacs-lisp
  (use-package org-agenda
    ...
    (org-agenda-files
     (list (file-name-as-directory (file-name-concat org-directory "agenda"))))
           ...
           (org-archive-location
            (cond
             ...
             ((member todo-state
              ...
              (format "%s::* suspended tasks" (file-name-concat org-directory "archive" (format-time-string "archive_%Y.org")))))))
...
[[https://github.com/skangas/mentor][mentor]] is a rTorrent client for emacs.
...
#+begin_src emacs-lisp :noweb tangle :tangle (expand-tangle-target ".emacs.d/site-lisp/lfile.el")
...
:PROPERTIES:
:CREATED:  [2022-10-10 Mon 20:47]
:END:

#+begin_src emacs-lisp
  (with-eval-after-load 'org-ql-view
    (push `("Suspended tasks"
            :buffers-files
            ,(file-expand-wildcards
              (file-name-concat org-directory "archive" (format-time-string "archive_*.org")))
            :query (parent (heading "suspended tasks"))
            :super-groups ((:auto-property "ARCHIVE_FILE"))
            :sort (priority date))
          org-ql-views))
#+end_src
...
#+begin_src shell :noweb tangle :tangle (expand-tangle-target ".local/bin/updatedb-home") :tangle-mode (identity #o755)
  #!/usr/bin/env bash
...
#+begin_src shell :shebang #!/bin/sh :tangle (expand-tangle-target ".local/bin/install-textlint") :tangle-mode (identity #o755)
  npm install -g \
    textlint \
...
#+begin_src shell :noweb tangle :tangle (expand-tangle-target ".local/bin/skkdic_uniq.sh") :shebang #!/usr/bin/env bash :tangle-mode (identity #o755)
  tmp_euc=$(mktemp)
  tmp_utf8=$(mktemp)
...
- a script to download voices
  ...
  #+begin_src shell :tangle (expand-tangle-target ".local/bin/download-festival-voices") :shebang #!/usr/bin/env bash :tangle-mode (identity #o755)
...
- download hts voices
  ...
  #+begin_src shell :noweb tangle :tangle (expand-tangle-target ".local/bin/download-hts-voices") #!/usr/bin/env bash :tangle-mode (identity #o755)
    ...
    curl -sL http://sourceforge.net/projects/mmdagent/files/MMDAgent_Example/MMDAgent_Example-1.8/MMDAgent_Example-1.8.zip \
      | busybox unzip -o -d ${data_dir} - \
      && ln -sfv ${data_dir}/MMDAgent_Example-1.8/Voice ${data_dir}/
  #+end_src
...
#+begin_src shell :noweb tangle :tangle (expand-tangle-target ".local/bin/sync_sv04") :shebang #!/usr/bin/env bash :tangle-mode (identity #o755)
...
#+begin_src conf :noweb tangle :tangle (expand-tangle-target ".rtorrent.rc")
...
#+begin_src conf :noweb tangle :tangle (expand-tangle-target ".config/systemd/user/rtorrent.service")
  [Unit]
...
#+begin_src bash :noweb tangle :tangle (expand-tangle-target ".local/bin/pwsudo")
  _userid=$(whoami)
  _hostname=$(hostname)
...
#+begin_src shell :tangle (expand-tangle-target ".local/bin/ghq-sync") :shebang #!/usr/bin/env bash :tangle-mode (identity #o500)
...
#+begin_src shell :noweb tangle :tangle (expand-tangle-target ".local/bin/git-sync") :tangle-mode (identity #o755)
  #!/usr/bin/env bash
  #
  # git-sync
  #
