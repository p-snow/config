...
if [ $DESKTOP_SESSION == 'plasma' ]; then
  guix shell python-pip -- pip install konsave && konsave -s <<login-name()>> -f
fi
#+end_src
...
(use-package org-src
  ...
  (org-src-lang-modes
   (mapc (lambda (lang-mode)
           (setf (alist-get (car lang-mode) (custom--standard-value 'org-src-lang-modes))
                 (cdr lang-mode)))
         '(("bash"   . bash-ts)
           ("C"      . c)
           ("ruby"   . ruby)
...
(defvar tempel-shell-command-template-dir
  (no-littering-expand-etc-file-name "tempel/templates/shell-command/*.eld"))
