...
- Automating Script
  ...
  #+end_src
  #+begin_src shell :eval no :tangle no :noweb-ref system-batch
    guix pull && systemctl restart guix-daemon.service
  #+end_src
...
#+begin_src shell :noweb tangle :tangle (expand-file-name ".local/bin/btrbk_home")
...
#+begin_src conf :noweb yes :noweb-ref cronjob-user
  ,*/15 * * * * <<which(filename="bash")>> -ci "which btrbk_home | xargs pwsudo >> $LOG_DIR/btrbk_home.log 2>&1"
#+end_src
...
#+begin_src emacs-lisp :noweb tangle
  (use-package midnight
    ...
    :custom
    (midnight-mode t)
    (midnight-delay "<<midnight-hour>>:03"))
#+end_src
      ...
      (cl-loop for entry in entries
               ...
               do (let ((title (elfeed-entry-title entry)))
                    (detached-shell-command
                     ;; FIXME: double-quote(") seems not to be processed in `detached-shell-command'
               ...
               (format "pwsudo rtcwake -v -m mem -l --date '%s'"
                       (string-trim
                        (shell-command-to-string (format "date +'%%Y-%%m-%%d %%H:%%M' -d '%s'"
                                                         (or (and (stringp wake-at) (length> wake-at 0) wake-at)
...
#+begin_src emacs-lisp
  (defun my/command-transcode-opus (&optional file)
    ...
    (interactive (list (if (eq minibuffer-history-variable 'dired-shell-command-history)
                           "*"
                         (read-file-name "File: "))))
    (format "parallel ffmpeg -i '{}' -c:a libopus -b:a 64k -ac 2 '{.}'.oga ::: %s"
            file))
#+end_src
...
- cron job for midnight.sh
  #+begin_src conf :noweb yes :noweb-ref cronjob-user
    2 <<midnight-hour>> * * <<weekdays()>> <<which(filename="bash")>> -ci "midnight.sh"
  #+end_src
...
#+begin_src emacs-lisp :noweb yes
  (let ((days nil)
    ...
    (dotimes (day 7)
      (unless (= day weekend)
        (add-to-list 'days (number-to-string day) t)))
    (string-join days ","))
#+end_src
...
#+begin_src conf :noweb yes :noweb-ref cronjob-user
  2 <<midnight-hour>> * * <<weekend-day>> <<which(filename="bash")>> -ci "which system.sh | xargs pwsudo; midnight.sh; weekend.sh"
#+end_src
...
#+begin_src shell :noweb tangle :tangle (expand-file-name ".local/bin/system.sh")
  <<system-batch>>
#+end_src
...
#+begin_src shell :noweb tangle :tangle (expand-file-name ".local/bin/pwsudo")
  _userid=$(whoami)
  _hostname=$(hostname)

  pas=$(pass show "${_hostname}/${_userid}" 2>/dev/null) \
    && ( echo "${pas}" | head -1 | sudo -iS -p "" "$@" ) \
    || sudo -iS "$@"
